...
namespace SebbyLib.Prediction
{
    ...
    public static class Prediction
    {
        ...
        internal static PredictionOutput GetPrediction(PredictionInput input, bool ft, bool checkCollision)
        {
            ...
            if (result.Hitchance >= HitChance.VeryHigh && input.Unit is AIHeroClient && input.Radius > 1)
            {
                ...
                OktwCommon.debug(input.Radius + " RES Ways: " + input.Unit.LSGetWaypoints().Count + " W " + input.Unit.IsHPBarRendered + " D " + distanceUnitToWaypoint + " T " + UnitTracker.GetLastNewPathTime(input.Unit) + " " + result.Hitchance);
            }
            return result;
        }
        ...
        internal static PredictionOutput GetPositionOnPath(PredictionInput input, List<Vector2> path, float speed = -1)
        {
            ...
            if (path.Count <= 1 || (input.Unit.IsHPBarRendered && !input.Unit.LSIsDashing()))
            {
                return new PredictionOutput
                {
                    Input = input,
    ...
    internal static class UnitTracker
    {
        ...
        static UnitTracker()
        {
            ...
            AttackableUnit.OnModifyShield += Obj_AI_Base_OnEnterLocalVisiblityClient;
        }
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender is AIHeroClient)
            {
                ...
                else
                {
                    ...
                    else if (sender.IsHPBarRendered || sender.IsRooted || !sender.CanMove)
                    {
