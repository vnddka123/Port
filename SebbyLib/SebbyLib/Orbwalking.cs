...
namespace SebbyLib
{
    public static class Orbwalking
    {
        ...
        private static void Obj_AI_Base_OnDelete(GameObject sender, EventArgs args)
        {
            ...
            if (DelayOnFire != 0 && missile != null && Player.AttackDelay > 1 / 2f)
            {
                if (missile.SpellCaster.IsMe && missile.SData.LSIsAutoAttack() && DelayOnFireId == missile.Target.NetworkId)
                {
                    ...
                    if (x < 115 - Game.Ping / 2)
                    {
                        BrainFarmInt -= 2;
                    }
                    else if (x > 135 - Game.Ping / 2)
                    {
                        BrainFarmInt += 2;
                    }
        ...
        public static bool CanMove(float extraWindup, bool disableMissileCheck = false)
        {
            ...
            return NoCancelChamps.Contains(_championName) || (Utils.GameTimeTickCount + Game.Ping / 2 >= LastAATick + Player.AttackCastDelay * 1000 + extraWindup + localExtraWindup) && !EloBuddy.Player.Instance.Spellbook.IsChanneling;
        }
            ...
            bool randomizeMinDistance = true)
        {
            ...
            try
            {
                if (target.LSIsValidTarget() /*&& CanAttack()*/ && Attack && EloBuddy.SDK.Orbwalker.CanAutoAttack)
                {
                    DisableNextAttack = false;
                ...
                if (/*CanMove(extraWindup) &&*/ Move && EloBuddy.SDK.Orbwalker.CanMove)
                {
        ...
        private static void OnProcessSpell(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs Spell)
        {
            try
            {
                ...
                if (unit.IsMe && IsAutoAttackReset(spellName) && Spell.SData.SpellCastTime == 45)
                {
                    ResetAutoAttackTimer();
                }
                    ...
                    mode == OrbwalkingMode.Freeze) && CanAttack())
                {
                    ...
                    if (closestTower != null && Player.LSDistance(closestTower, true) < 1500 * 1500)
                    {
                        ...
                        var minions = MinionListAA.Where(minion =>
                            closestTower.LSDistance(minion, true) < 900 * 900)
