    ...
    internal class Ryze : Helper
    {
        ...
        private static void OnPossibleToInterrupt(AIHeroClient unit, Interrupter2.InterruptableTargetEventArgs spell)
        {
            if (Player.IsDead || !GetValue<bool>("Interrupt", "W") ||
                !GetValue<bool>("Interrupt", unit.ChampionName + "_" + spell) || !W.CanCast(unit))
            {
                return;
            }
            W.CastOnUnit(unit);
        ...
        private static void Fight(string mode)
        {
            if (mode == "Harass")
            {
                ...
                if (GetValue<bool>(mode, "W") && W.CastOnBestTarget(0).IsCasted())
                {
                    return;
                }
                if (GetValue<bool>(mode, "E"))
                {
                    E.CastOnBestTarget(0);
                }
            }
            else
            {
                ...
                {
                    if (Q.CastOnBestTarget(0).IsCasted())
                    {
                        return;
                    }
                    if (W.CastOnBestTarget(0).IsCasted())
                    {
                        return;
                    }
                    E.CastOnBestTarget(0);
        ...
        private static void KillSteal()
        {
            ...
            if (GetValue<bool>("KillSteal", "W") && W.IsReady())
            {
                ...
                if (target != null && W.IsKillable(target) && W.CastOnUnit(target))
                {
                    return;
                }
            }
