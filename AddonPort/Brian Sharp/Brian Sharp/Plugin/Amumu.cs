    ...
    internal class Amumu : Helper
    {
        ...
        private static void OnPossibleToInterrupt(AIHeroClient unit, Interrupter2.InterruptableTargetEventArgs spell)
        {
            if (Player.IsDead || !GetValue<bool>("Interrupt", "Q") ||
                !GetValue<bool>("Interrupt", unit.ChampionName + "_" + spell) || !Q.CanCast(unit))
            {
                return;
            }
            Q.Cast(unit);
        ...
        private static void Fight(string mode)
        {
                ...
                if (GetValue<bool>(mode, "Q") && Q.IsReady())
                {
                    if (GetValue<bool>(mode, "R") && R.IsReady(100))
                    {
                        ...
                        if ((from i in nearObj
                            ...
                            select i).Any(i => Q.Cast(i).IsCasted()))
                        {
                            return;
                        }
                    }
            ...
            if (GetValue<bool>(mode, "E") && E.IsReady() && E.GetTarget() != null && E.Cast())
            {
                return;
            }
        ...
        private static void Clear()
        {
            ...
            if (GetValue<bool>("Clear", "E") && E.IsReady() && minionObj.Any(i => E.IsInRange(i)) && E.Cast())
            {
                return;
            }
            ...
            {
                var obj =
                    minionObj.Where(i => Q.IsKillable(i) || !Orbwalking.InAutoAttackRange(i))
