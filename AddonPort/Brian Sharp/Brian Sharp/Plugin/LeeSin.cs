    ...
    internal class LeeSin : Helper
    {
        ...
        private static void Star()
        {
            ...
            if (Q.IsReady())
            {
                if (IsQOne)
                {
                    if (R.IsReady() && Q.Cast(target).IsCasted())
                    {
                        return;
                    }
                }
                         ...
                         (Q.IsKillable(target, 1) ||
                          (!R.IsReady() && Utils.TickCount - R.LastCastAttemptT > 300 &&
                           Utils.TickCount - R.LastCastAttemptT < 1500)) && Q2.Cast())
                {
                    return;
                }
            }
            if (E.IsReady() && IsEOne && E.IsInRange(target) && HaveQ(target) && !R.IsReady() &&
                Utils.TickCount - R.LastCastAttemptT < 1500 && Player.Mana >= 80 && E.Cast())
            {
                return;
            }
        ...
        {
            if (GetValue<bool>(mode, "P") && HaveP && Orbwalking.GetBestHeroTarget != null && Orbwalking.CanAttack())
            {
                return;
            }
                ...
                else
                {
                    ...
                    if (target != null &&
                        (QAgain(target) ||
                         ...
                         Q.IsKillable(target, 1) || !Orbwalking.InAutoAttackRange(target, 100) || !HaveP) &&
                        Q2.Cast())
                    {
                        return;
                    }
            ...
            if (GetValue<bool>(mode, "E") && E.IsReady())
            {
                if (IsEOne)
                {
                    ...
                    HeroManager.Enemies.Where(i => i.IsValidTarget(E2.Range) && HaveE(i))
                        .Any(i => EAgain(i) || !Orbwalking.InAutoAttackRange(i, 50) || !HaveP) && Player.Mana >= 50 &&
                    E2.Cast())
                {
                    return;
                }
            ...
            if (GetValue<bool>(mode, "R") && R.IsReady())
            {
                ...
                {
                    if (CanKill(target, GetQ2Dmg(target, R.GetDamage(target))) && R.CastOnUnit(target))
                    {
                        return;
                    }
                }
            ...
            if (GetValue<bool>(mode, "W") && W.IsReady() && Orbwalking.GetBestHeroTarget != null)
            {
                if (IsWOne)
                {
        ...
        private static void Flee(Vector3 pos)
        {
            if (!GetValue<bool>("Flee", "W") || !W.IsReady() || !IsWOne ||
                Utils.GameTimeTickCount - _limitWardTime <= 50)
            {
                return;
            }
        ...
        private static void KillSteal()
        {
            ...
            if (GetValue<bool>("KillSteal", "Q") && Q.IsReady())
            {
                ...
                {
                    var target = Q2.GetTarget(0, HeroManager.Enemies.Where(i => !HaveQ(i)));
                    if (target != null && Q.IsKillable(target, 1) && Q2.Cast())
                    {
                        return;
                    }
                }
                    ...
                    if (R.IsInRange(Target) && Player.Distance(PosAfterKick) > Target.Distance(PosAfterKick) &&
                        PosAfterKick.Distance(
                            PosAfterKick.To2D()
                                ...
                                .LinePoint.To3D()) < RKickRange * 0.5f && R.CastOnUnit(Target))
                    {
                        return;
                    }
