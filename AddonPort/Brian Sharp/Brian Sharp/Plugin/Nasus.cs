...
namespace BrianSharp.Plugin
{
    internal class Nasus : Helper
    {
        ...
        private static void AfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            ...
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Mixed && GetValue<bool>("Harass", "Q") && target is AIHeroClient &&
                Q.Cast())
            {
        ...
        private static void Fight(string mode)
        {
            if (mode == "Combo")
            {
                ...
                {
                    var target = Orbwalking.GetBestHeroTarget;
                    if (target != null)
                    {
                        if (!HaveQ)
                        {
            ...
            if (GetValue<bool>(mode, "E") && E.IsReady())
            {
                ...
                if (target != null && (mode == "Combo" || Orbwalking.InAutoAttackRange(target, 50)) &&
            ...
            if (GetValue<bool>(mode, "W") && W.IsReady())
            {
                ...
                if (target != null &&
                    ((mode == "Combo" && (!Orbwalking.InAutoAttackRange(target, 50) || target.HealthPercent > 30)) ||
                ...
                var obj =
                    (Obj_AI_Base)
                        ObjectManager.Get<Obj_AI_Turret>()
                            .FirstOrDefault(i => Orbwalking.InAutoAttackRange(i) && CanKill(i, GetBonusDmg(i))) ??
