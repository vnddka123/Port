    ...
    internal class Maokai : Helper
    {
        ...
        private static void OnPossibleToInterrupt(AIHeroClient unit, Interrupter2.InterruptableTargetEventArgs spell)
        {
            ...
            if (Player.Distance(unit) > QKnockUpWidth && W.CanCast(unit) &&
                Player.Mana >= Q.Instance.SData.Mana + W.Instance.SData.Mana && W.CastOnUnit(unit))
            {
                return;
            }
        ...
        private static void Fight(string mode)
        {
            if (mode == "Combo" && GetValue<bool>(mode, "R") && R.IsReady())
            {
                ...
                if (!Player.HasBuff("MaokaiDrain3"))
                {
                    if (Player.ManaPercent >= GetValue<Slider>(mode, "RMpA").Value &&
                        (obj.Any(i => i.HealthPercent < GetValue<Slider>(mode, "RHpU").Value) ||
                         obj.Count >= GetValue<Slider>(mode, "RCountA").Value) && R.Cast())
                    {
                        return;
                    }
                }
                else
                {
                    ...
                    if (Player.ManaPercent < GetValue<Slider>(mode, "RMpA").Value && R.Cast())
                    {
                        return;
                    }
                }
            ...
            if (mode == "Gank")
            {
                ...
                if (W.IsReady())
                {
                    if (E.IsReady() && E.Cast(target).IsCasted())
                    {
                        return;
                    }
                    W.CastOnUnit(target);
            ...
            {
                if (GetValue<bool>(mode, "E") && E.CastOnBestTarget(E.Width / 2, true).IsCasted())
                {
                    return;
                }
                if (GetValue<bool>(mode, "W") &&
                    (mode == "Combo" || Player.HealthPercent >= GetValue<Slider>(mode, "WHpA").Value) &&
                    W.CastOnBestTarget(0).IsCasted())
                {
                    return;
                }
        ...
        private static void Clear()
        {
            ...
            if (GetValue<bool>("Clear", "W") && W.IsReady())
            {
                ...
                if (obj == null && !minionObj.Any(i => Orbwalking.InAutoAttackRange(i, 40)))
                {
