...
namespace BrianSharp.Plugin
{
    internal class JarvanIV : Helper
    {
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (R.IsReady() && _rCasted && Player.CountEnemiesInRange(RWidth) == 0 && R.Cast())
            {
                return;
            }
        ...
        private static void OnPossibleToInterrupt(AIHeroClient unit, Interrupter2.InterruptableTargetEventArgs spell)
        {
            if (Player.IsDead || !GetValue<bool>("Interrupt", "EQ") ||
                !GetValue<bool>("Interrupt", unit.ChampionName + "_" + spell) || !Q.IsReady())
            {
                return;
            }
            if (E.CanCast(unit) && Player.Mana >= Q.Instance.SData.Mana + E.Instance.SData.Mana)
            {
                ...
                if (predE.Hitchance >= E.MinHitChance &&
                    E.Cast(
                        predE.UnitPosition.Extend(Player.ServerPosition, -E.Width / (unit.IsFacing(Player) ? 2 : 1))) && Q.Cast(predE.UnitPosition))
                {
                    return;
                }
            }
                    ...
                    if (predE.Hitchance >= E.MinHitChance &&
                        E.Cast(
                            predE.UnitPosition.Extend(
                                Player.ServerPosition, -E.Width / (target.IsFacing(Player) ? 2 : 1))))
                    {
                        if (GetValue<bool>(mode, "Q") && Q.IsReady())
                        {
                            Q.Cast(predE.UnitPosition);
                        }
                        return;
                    }
                }
                    ...
                    if (GetValue<bool>(mode, "E") && target != null &&
                        Flag.Where(
                            i =>
                                ...
                                Q2.WillHit(target, i.ServerPosition)).Any(i => Q.Cast(i.ServerPosition)))
                    {
                        return;
                    }
                }
                if (Q.CastOnBestTarget(0, true).IsCasted())
                {
                    return;
                }
            }
        ...
        private static void Clear()
        {
            ...
            if (GetValue<bool>("Clear", "E") && E.IsReady())
            {
                ...
                if (minionObj.Any())
                {
                    ...
                    else
                    {
                        ...
                        if (obj != null && E.Cast(obj).IsCasted())
                        {
                            return;
                        }
                    }
            ...
            if (GetValue<bool>("Clear", "W") && W.IsReady() &&
                ...
                GetMinions(W.Range, MinionTypes.All, MinionTeam.NotAlly).Any() && W.Cast())
            {
                return;
            }
        ...
        private static void Flee()
        {
            if (GetValue<bool>("Flee", "EQ") && Q.IsReady() && E.IsReady() &&
                Player.Mana >= Q.Instance.SData.Mana + E.Instance.SData.Mana && E.Cast(Game.CursorPos) &&
        ...
        private static void KillSteal()
        {
            ...
            if (GetValue<bool>("KillSteal", "Q") && Q.IsReady())
            {
                ...
                if (target != null && Q.IsKillable(target) && Q.Cast(target).IsCasted())
                {
                    return;
                }
            }
            if (GetValue<bool>("KillSteal", "E") && E.IsReady())
            {
                ...
                if (target != null && E.IsKillable(target) && E.Cast(target).IsCasted())
                {
                    return;
                }
            }
