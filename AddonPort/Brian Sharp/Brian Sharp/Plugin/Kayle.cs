...
namespace BrianSharp.Plugin
{
    internal class Kayle : Helper
    {
        ...
        private static void Fight(string mode)
        {
            ...
            {
                var target =
                    HeroManager.Enemies.Where(i => Orbwalking.InAutoAttackRange(i))
            ...
            if (GetValue<bool>(mode, "W") && GetValue<bool>(mode, "WSpeed") && W.IsReady())
            {
                ...
                if (target != null && !target.IsFacing(Player) && (!HaveE || !Orbwalking.InAutoAttackRange(target)) &&
                    (!GetValue<bool>(mode, "Q") || (Q.IsReady() && !Q.IsInRange(target))) && W.Cast())
                {
                    return;
                }
            }
        ...
        {
            if (GetValue<bool>("Flee", "W") && W.IsReady() && W.Cast())
            {
                return;
            }
                ...
                foreach (var buff in obj.Buffs)
                {
                        ...
                        (buff.DisplayName == "VladimirHemoplagueDebuff" && GetValue<bool>("Anti", "Vlad")))
                    {
                        RAntiDetected.Add(obj.NetworkId, new RAntiItem(buff));
                             ...
                             obj.Health <=
                             ((AIHeroClient) buff.Caster).GetSpellDamage(obj, SpellSlot.R) + obj.Health * 0.2f &&
