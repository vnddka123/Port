...
namespace SFXUtility.Features.Timers
{
    internal class Cooldown : Child<Timers>
    {
        ...
        private void OnObjAiBaseTeleport(Obj_AI_Base sender, Teleport.TeleportEventArgs args)
        {
            try
            {
            ...
            try
            {
                        ...
                        hero => hero != null && hero.IsValid && hero.IsHPBarRendered && hero.Position.IsOnScreen()))
                {
                    try
                    {
                        ...
                        var x = (int) hero.HPBarPosition.X - (hero.IsMe ? -10 : 15);
                        var y = (int) hero.HPBarPosition.Y + (hero.IsEnemy ? 3 : (hero.IsMe ? 6 : 14));
