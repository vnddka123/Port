    ...
    class Program
    {
        ...
        private static void Qlogic()
        {
            ...
            else
            {
                ...
                if (t.LSIsValidTarget())
                {
                    if ((!Orbwalking.InAutoAttackRange(t) || t.LSCountEnemiesInRange(250) >= Config.Item("Qaoe").GetValue<Slider>().Value))
                    {
                        ...
                        if (Farm && Orbwalking.CanAttack() && !Player.Spellbook.IsAutoAttacking && Config.Item("harasQ" + t.ChampionName).GetValue<bool>() && Config.Item("Qharass").GetValue<bool>() && (Player.ManaPercent > Config.Item("QmanaHarass").GetValue<Slider>().Value || Player.LSGetAutoAttackDamage(t) * Config.Item("QmanaIgnore").GetValue<Slider>().Value > t.Health))
                        {
                            Q.Cast();
                        }
                    }
