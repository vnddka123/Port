...
namespace HoolaRiven
{
    public class Program
    {
        ...
        private static readonly SpellSlot Flash = EloBuddy.Player.Instance.GetSpellSlot("summonerFlash");
        private static Spell Q, W, E, R;
        private static int QStack = 1;
   //     private static Text Timer, Timer2;
   //     private static Text Timer = new Text(string.Empty, new System.Drawing.Font(System.Drawing.FontFamily.GenericSansSerif, 8, System.Drawing.FontStyle.Regular));
   //     private static Text Timer2 = new Text(string.Empty, new System.Drawing.Font(System.Drawing.FontFamily.GenericMonospace, 8, System.Drawing.FontStyle.Bold));
        ...
        private static void Main(string[] args) => CustomEvents.Game.OnGameLoad += OnGameLoad;
        ...
        private static void OnGameLoad(EventArgs args)
        {
            ...
            //Timer = new Text("Q Expiry =>  " + ((double)(LastQ - Utils.GameTimeTickCount + 3800) / 1000).ToString("0.0"), new System.Drawing.Font(System.Drawing.FontFamily.GenericSansSerif, 8, System.Drawing.FontStyle.Regular));
            //Timer2 = new Text("R Expiry =>  " + (((double)LastR - Utils.GameTimeTickCount + 15000) / 1000).ToString("0.0"), new System.Drawing.Font(System.Drawing.FontFamily.GenericMonospace, 8, System.Drawing.FontStyle.Bold));
      ...
      private static void OnDoCastLC(Obj_AI_Base Sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (args.Target is Obj_AI_Minion)
            {
                ...
                {
                    var Minions = MinionManager.GetMinions(70 + 120 + EloBuddy.Player.Instance.BoundingRadius);
                    if (HasTitan())
                    {
                        CastTitan();
                        return;
      ...
      private static void OnDoCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (args.Target is Obj_AI_Minion)
            {
                if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LaneClear)
                {
                    var Mobs = MinionManager.GetMinions(120 + 70 + EloBuddy.Player.Instance.BoundingRadius, MinionTypes.All,
            ...
            var target = (AIHeroClient)args.Target;
                if (KillstealR && R.IsReady() && R.Instance.Name == IsSecondR) if (target.Health < (Rdame(target, target.Health) + EloBuddy.Player.Instance.GetAutoAttackDamage(target)) && target.Health > EloBuddy.Player.Instance.GetAutoAttackDamage(target)) R.Cast(target.Position);
                if (KillstealW && W.IsReady()) if (target.Health < (W.GetDamage(target) + EloBuddy.Player.Instance.GetAutoAttackDamage(target)) && target.Health > EloBuddy.Player.Instance.GetAutoAttackDamage(target)) W.Cast();
...
//            Draw.AddItem(new MenuItem("DrawTimer2", "Draw R Expiry Time").SetValue(true));
      ...
      private static void Interrupt(AIHeroClient sender, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            {
                if (sender.IsValidTarget(125 + EloBuddy.Player.Instance.BoundingRadius + sender.BoundingRadius)) W.Cast();
            }
        }

        private static int GetWRange => EloBuddy.Player.Instance.HasBuff("RivenFengShuiEngine") ? 330 : 265;

      private static void AutoUseW()
        {
            if (AutoW > 0)
            {
                if (EloBuddy.Player.Instance.CountEnemiesInRange(GetWRange) >= AutoW)
                {
                    ForceW();
                }
            }
      ...
      //      if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.FastHarass) FastHarass();
            ...
            if (Utils.GameTimeTickCount - LastQ >= 3650 && QStack != 1 && !EloBuddy.Player.Instance.IsRecalling() && KeepQ && Q.IsReady()) Q.Cast(Game.CursorPos);
        }
      ...
      private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                Timer.TextValue = ("Q Expiry =>  " + ((double)(LastQ - Utils.GameTimeTickCount + 3800) / 1000).ToString("0.0")+"S");
                Timer.Position = new Vector2((int)Drawing.WorldToScreen(ObjectManager.Player.Position).X - 60, (int)Drawing.WorldToScreen(ObjectManager.Player.Position).Y + 43);
                Timer.Draw();
            }

            if (EloBuddy.Player.Instance.HasBuff("RivenFengShuiEngine") && DrawTimer2)
            {
                Timer2.TextValue = ("R Expiry =>  " + (((double)LastR - Utils.GameTimeTickCount + 15000) / 1000).ToString("0.0") +"S");
                Timer2.Position = new Vector2((int)Drawing.WorldToScreen(ObjectManager.Player.Position).X - 60, (int)Drawing.WorldToScreen(ObjectManager.Player.Position).Y + 65);
                Timer2.Draw();
            }
            //*/
            if (DrawCB) Render.Circle.DrawCircle(EloBuddy.Player.Instance.Position, 250 + EloBuddy.Player.Instance.AttackRange + 70, E.IsReady() ? System.Drawing.Color.FromArgb(120, 0, 170, 255) : System.Drawing.Color.IndianRed);
            if (DrawBT && Flash != SpellSlot.Unknown) Render.Circle.DrawCircle(EloBuddy.Player.Instance.Position, 800, R.IsReady() && Flash.IsReady() ? System.Drawing.Color.FromArgb(120, 0, 170, 255) : System.Drawing.Color.IndianRed);
            if (DrawFH) Render.Circle.DrawCircle(EloBuddy.Player.Instance.Position, 450 + EloBuddy.Player.Instance.AttackRange + 70, E.IsReady() && Q.IsReady() ? System.Drawing.Color.FromArgb(120, 0, 170, 255) : System.Drawing.Color.IndianRed);
            if (DrawHS) Render.Circle.DrawCircle(EloBuddy.Player.Instance.Position, 400, Q.IsReady() && W.IsReady() ? System.Drawing.Color.FromArgb(120, 0, 170, 255) : System.Drawing.Color.IndianRed);
            if (DrawAlwaysR) 
            {
      ...
      private static void Jungleclear()
        {
            ...
            var Mobs = MinionManager.GetMinions(250 + EloBuddy.Player.Instance.AttackRange + 70, MinionTypes.All, MinionTeam.Neutral, MinionOrderTypes.MaxHealth);
      ...
      private static void Combo()
        {
            var targetR = TargetSelector.GetTarget(250 + EloBuddy.Player.Instance.AttackRange + 70, TargetSelector.DamageType.Physical);
      ...
      private static void Burst()
        {
            ...
            {
                if (R.IsReady() && R.Instance.Name == IsFirstR && W.IsReady() && E.IsReady() && EloBuddy.Player.Instance.Distance(target.Position) <= 250 + 70 + Player.AttackRange)
                {
                    E.Cast(target.Position);
                    CastYoumoo();
                    ForceR();
                    Utility.DelayAction.Add(100, ForceW);
                         ...
                         EloBuddy.Player.Instance.Distance(target.Position) <= 400 + 70 + EloBuddy.Player.Instance.AttackRange)
                {
                    E.Cast(target.Position);
                    CastYoumoo();
                    ForceR();
                    ...
                    && R.IsReady() && R.Instance.Name == IsFirstR && (EloBuddy.Player.Instance.Distance(target.Position) <= 800) && (!FirstHydra || (FirstHydra && !HasItem())))
                {
                    E.Cast(target.Position);
                    CastYoumoo();
                    ForceR();
                    ...
                    && R.IsReady() && E.IsReady() && W.IsReady() && R.Instance.Name == IsFirstR && (EloBuddy.Player.Instance.Distance(target.Position) <= 800) && FirstHydra && HasItem())
                {
                    E.Cast(target.Position);
                    ForceR();
      ...
      private static void FastHarass()
        {
            ...
            {
                var target = TargetSelector.GetTarget(450 + EloBuddy.Player.Instance.AttackRange + 70, TargetSelector.DamageType.Physical);
      ...
      private static void Harass()
        {
            ...
            {
                var epos = EloBuddy.Player.Instance.ServerPosition +
                          (EloBuddy.Player.Instance.ServerPosition - target.ServerPosition).Normalized() * 300;
                E.Cast(epos);
      ...
      private static void Flee()
        {
            var enemy =
                HeroManager.Enemies.Where(
                    hero =>
                        hero.IsValidTarget(EloBuddy.Player.Instance.HasBuff("RivenFengShuiEngine")
                            ? 70 + 195 + EloBuddy.Player.Instance.BoundingRadius
                            : 70 + 120 + EloBuddy.Player.Instance.BoundingRadius) && W.IsReady());
            var x = EloBuddy.Player.Instance.Position.Extend(Game.CursorPos, 300);
            if (W.IsReady() && enemy.Any()) foreach (var target in enemy) if (InWRange(target)) W.Cast();
            if (Q.IsReady() && !EloBuddy.Player.Instance.IsDashing()) Q.Cast(Game.CursorPos);
            if (E.IsReady() && !EloBuddy.Player.Instance.IsDashing()) E.Cast(x);
        }
      ...
      private static void Reset()
        {
            Chat.Say("/d");
            Orbwalking.LastAATick = 0;
            EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, EloBuddy.Player.Instance.Position.Extend(Game.CursorPos, EloBuddy.Player.Instance.Distance(Game.CursorPos) + 10));
        }

      private static bool InWRange(GameObject target)=> (Player.HasBuff("RivenFengShuiEngine") && target != null) ? 330 >= EloBuddy.Player.Instance.Distance(target.Position) : 265 >= EloBuddy.Player.Instance.Distance(target.Position);
        

        private static void ForceSkill()
        {
            if (forceQ && QTarget != null && QTarget.IsValidTarget(E.Range + EloBuddy.Player.Instance.BoundingRadius + 70) && Q.IsReady()) Q.Cast(QTarget.Position);
            if (forceW) W.Cast();
      ...
      private static void FlashW()
        {
            ...
            {
                W.Cast();
                Utility.DelayAction.Add(10, () => EloBuddy.Player.Instance.Spellbook.CastSpell(Flash, target.Position));
            }
        }
      ...
      private static void OnCasting(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsEnemy && sender.Type == EloBuddy.Player.Instance.Type && (AutoShield || (Shield && Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LastHit)))
            {
                var epos = EloBuddy.Player.Instance.ServerPosition +
                          (EloBuddy.Player.Instance.ServerPosition - sender.ServerPosition).Normalized() * 300;

                if (EloBuddy.Player.Instance.Distance(sender.ServerPosition) <= args.SData.CastRange)
                {
                    switch (args.SData.TargettingType)
                    {
                        case SpellDataTargetType.Unit:

                            if (args.Target.NetworkId == EloBuddy.Player.Instance.NetworkId)
                            {
