...
namespace SPrediction
{
    ...
    internal class PathTracker
    {
        ...
        {
            foreach (AIHeroClient enemy in HeroManager.Enemies) EnemyInfo.Add(enemy.NetworkId, new EnemyData(new List<Vector2>()));

            Obj_AI_Base.OnNewPath += new Obj_AI_BaseNewPath(AIHeroClient_OnNewPath);
            Obj_AI_Base.OnSpellCast += new Obj_AI_BaseDoCastSpell(AIHeroClient_OnDoCast);
            Obj_AI_Base.OnSpellCast += new Obj_AI_BaseDoCastSpell(AIHeroClient_OnProcessSpellCast);
        }
