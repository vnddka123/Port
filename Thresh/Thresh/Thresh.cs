...
//		public static Orbwalking.Orbwalker Orbwalker;
        ...
        private static void LoadMenu()
        {
            ...
            FleeConfig.Add("flee", new KeyBind("Flee", false, KeyBind.BindTypes.HoldActive, 'T'));
       ...
       //     var target = TargetSelector.GetTarget(Q.Range, DamageType.Magical);
            ...
            if (QT != null && QT.IsValid)
			{
                ...
                if (QT.HasBuffOfType(BuffType.Stun) || QT.HasBuffOfType(BuffType.Knockup) || QT.HasBuffOfType(BuffType.Fear) || QT.HasBuffOfType(BuffType.Slow))
                    SpellQ.CastQ1(QT);
                //Q2逻辑
                if (SpellQ.GetState() == QState.threshqleap && QTarget.Position.CountEnemiesInRange(700) - Player.Position.CountEnemiesInRange(700) <= getSliderItem(SpellConfig, "dQ2if"))
				{
        ...
        private static void Unit_OnDash(Obj_AI_Base sender, Dash.DashItem args)
        {
			...
			if (sender.IsEnemy && Player.Distance(args.EndPos) > Player.Distance(args.StartPos))
			{
				...
				if (getCheckBoxItem(SpellConfig, "autoQ") && SpellQ.GetState() == QState.ThreshQ && Q.IsInRange(args.EndPos)&& !E.IsInRange(args.EndPos) && Math.Abs(args.Duration - args.EndPos.Distance(sender) / Q.Speed * 1000) < 150)
				{
