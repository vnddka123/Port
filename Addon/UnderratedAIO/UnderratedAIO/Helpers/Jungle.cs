...
namespace UnderratedAIO.Helpers
{
    public class Jungle
    {
        ...
        public static Obj_AI_Base GetNearest(Vector3 pos, float range = 1500f)
        {
            return
                MinionManager.GetMinions(pos, range, MinionTypes.All, MinionTeam.NotAlly)
                    .FirstOrDefault(
                        minion =>
                            minion.IsValidTargetLS() && minion.IsValid && minion.Distance(pos) < range &&
        ...
        {
            return player.GetSummonerSpellDamage(target, LeagueSharp.Common.Damage.SummonerSpell.Smite);
        }
