...
namespace UnderratedAIO.Helpers
{
    public class ItemHandler
    {
        ...
        public static void UseItems(AIHeroClient target, Menu config, float comboDmg = 0f, bool cleanseSpell = false)
        {
            ...
            {
                var odinDmg = LeagueSharp.Common.Damage.GetItemDamageLS(player, target, LeagueSharp.Common.Damage.DamageItems.OdingVeils);
            ...
            {
                var bilDmg = LeagueSharp.Common.Damage.GetItemDamageLS(player, target, LeagueSharp.Common.Damage.DamageItems.Bilgewater);
            ...
            {
                var botrDmg = LeagueSharp.Common.Damage.GetItemDamageLS(player, target, LeagueSharp.Common.Damage.DamageItems.Botrk);
            ...
            {
                var hexDmg = LeagueSharp.Common.Damage.GetItemDamageLS(player, target, LeagueSharp.Common.Damage.DamageItems.Hexgun);
            ...
            if (config.Item("Zhonya").GetValue<bool>())
            {
                if (((config.Item("Zhonyadmg").GetValue<Slider>().Value / 100f * player.Health <=
                     ...
                     (Danger() && player.HealthPercent < 30)) && player.IsValidTargetLS(10, false))
                {
        ...
        public static bool IsHeRunAway(AIHeroClient target)
        {
            return (!target.IsFacing(player) &&
                    LeagueSharp.Common.Prediction.GetPrediction(target, 600, 100f).CastPosition.Distance(player.Position) >
        ...
        public static void castHydra(AIHeroClient target)
        {
            ...
            {
                if (Items.HasItem(tiamat.Id) && Items.CanUseItem(tiamat.Id) && !EloBuddy.SDK.Orbwalker.CanAutoAttack)
                {
                    Items.UseItem(tiamat.Id);
                }
                if (Items.HasItem(hydra.Id) && Items.CanUseItem(hydra.Id) && !EloBuddy.SDK.Orbwalker.CanAutoAttack)
                {
