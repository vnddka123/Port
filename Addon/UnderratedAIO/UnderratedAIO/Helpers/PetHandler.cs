    ...
    internal class PetHandler
    {
        ...
        internal static Menu addItemOptons(Menu menuM)
        {
            ...
            menuP.AddItem(new MenuItem("petTarget", "Pet target priority", true))
                .SetValue(new StringList(new[] { "LeagueSharp.Common.TargetSelector", "Lowest health", "Closest to you" }, 0));
        ...
        public static void MovePet(Menu options, Orbwalking.OrbwalkingMode orbwalkingMode)
        {
                ...
                (isCombo || isFarm || isLastHit))
            {
                ...
                if (gtarget == null)
                {
                    ...
                    {
                        var movePos = player.Position.Extend(LeagueSharp.Common.Prediction.GetPrediction(player, 0.5f).UnitPosition, -250).To3DWorld();
                        MoveTo(movePos);
                        SetPetDelay();
                    }
                }
                else if (gtarget.IsValid && !Pet.Spellbook.IsAutoAttacking)
                {
                    ...
                    else
                    {
                        ...
                        if (isLastHit || isFarm)
                        {
                            pos = gtarget.Position.Extend(Pet.Position, Pet.AttackRange).To3DWorld();
                        }
                        MoveTo(pos);
                    }
                    SetPetDelay();
        ...
        private static AttackableUnit GetTarget(bool isCombo, bool isFarm, bool isLastHit, int targetMode)
        {
            ...
            if (isCombo)
            {
                ...
                {
                    case 0:
                        gtarget = LeagueSharp.Common.TargetSelector.GetTarget(range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
                        break;
                    case 1:
                        gtarget =
                    ...
                    MinionManager.GetMinions(1000, MinionTypes.All, MinionTeam.NotAlly)
                        ...
                        .Concat(
                            ObjectManager.Get<Obj_AI_Turret>()
                                .Where(t => t.IsValidTargetLS() && t.Position.Distance(player.Position) < range))
                        .ToList();
                if (isFarm)
                {
                    gtarget =
        ...
        private static Vector3 GetMovementPos(AttackableUnit Gtarget, bool IsSafe)
        {
            ...
            try
            {
                predictionPos = LeagueSharp.Common.Prediction.GetPrediction((Obj_AI_Base) Gtarget, 2).UnitPosition;
            }
            catch (Exception)
            {
            ...
            if (!Pet.IsMelee)
            {
                ...
                {
                    pos = Gtarget.Position.Extend(player.Position, Pet.AttackRange).To3DWorld();
                }
                else
                {
                    var safePos =
                        CombatHelper.PointsAroundTheTargetOuterRing(
                            ...
                            .FirstOrDefault();
                    pos = Gtarget.Position.Extend(safePos, Pet.AttackRange).To3DWorld();
                }
            }
            else
            {
                if (IsSafe)
                {
                    pos = Gtarget.Position.Extend(predictionPos, Pet.AttackRange).To3DWorld();
                }
                else
                {
                    pos = Gtarget.Position.Extend(player.Position, Pet.AttackRange).To3DWorld();
