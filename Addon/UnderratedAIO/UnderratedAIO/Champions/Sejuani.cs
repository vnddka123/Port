...
namespace UnderratedAIO.Champions
{
    internal class Sejuani
    {
        ...
        private void OnPossibleToInterrupt(AIHeroClient unit, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            {
                if (unit.IsValidTargetLS(Q.Range) && Q.IsReady() && me.Distance(unit) < Q.Range)
                {
            ...
            {
                if (unit.IsValidTargetLS(R.Range) && R.IsReady() && me.Distance(unit) < R.Range)
                {
        ...
        private static void OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            {
                if (gapcloser.Sender.IsValidTargetLS(Q.Range) && Q.IsReady() && me.Distance(gapcloser.End) < Q.Range)
                {
            ...
            {
                if (gapcloser.Sender.IsValidTargetLS(R.Range) && R.IsReady() && me.Distance(gapcloser.End) < R.Range)
                {
            ...
            else
            {
                foreach (var enemy in
                    HeroManager.Enemies.Where(
                        i =>
                            i.IsValidTargetLS() && me.Distance(i) < R.Range &&
