        ...
        private void Rmovement()
        {
            if (rActive && Game.CursorPos.CountEnemiesInRange(300) > 1)
            {
                ...
                if (target != null && target.CountEnemiesInRange(R.Range) > 1)
                {
                    ...
                    {
                        var enemies =
                            HeroManager.Enemies.Where(e => e.IsValidTargetLS() && e.Distance(player) < 600)
        ...
        private static float ComboDamage(AIHeroClient hero)
        {
            ...
            if (Q.IsReady())
            {
                damage += player.GetAutoAttackDamage(hero) + Q.GetDamage(hero) +
                          Damage.GetSpellDamageLS(player, hero, SpellSlot.Q);
            }
            if (E.IsReady())
            {
            ...
            if (R.IsReady())
            {
                damage += Damage.GetSpellDamageLS(player, hero, SpellSlot.R) * 4;
            }
