...
namespace UnderratedAIO.Champions
{
    internal class Fiora
    {
        ...
        private void GameObject_OnCreate(GameObject sender, EventArgs args)
        {
            ...
            {
                var enemy =
                    HeroManager.Enemies.Where(e => e.IsValidTargetLS() && e.Distance(sender.Position) < 50)
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            {
                AIHeroClient enemy = LeagueSharp.Common.TargetSelector.GetTarget(W.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
        ...
        private void AfterAttack(AttackableUnit unit, AttackableUnit targetO)
        {
            ...
            if (unit.IsMe && E.IsReady() && orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo &&
                ...
                !EloBuddy.SDK.Orbwalker.CanAutoAttack)
            {
                E.Cast();
            }
            if (unit.IsMe && orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo && Q.IsReady() &&
                (config.Item("RapidAttack", true).GetValue<KeyBind>().Active || rapid) && !EloBuddy.SDK.Orbwalker.CanAutoAttack &&
                passivePositions.Any())
            {
                ...
                {
                    var pos2 = GetQpoint(targ, LeagueSharp.Common.Prediction.GetPrediction(targ, 2).UnitPosition);
                    if (pos2.IsValid())
                    {
                        Q.Cast(pos2);
                    }
        ...
        public static void Game_ProcessSpell(Obj_AI_Base hero, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            AIHeroClient targetW = LeagueSharp.Common.TargetSelector.GetTarget(W.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
        ...
        public Vector3 GetQpoint(AIHeroClient target, Vector3 passive)
        {
            ...
            var predEnemy = LeagueSharp.Common.Prediction.GetPrediction(target, ObjectManager.Player.Distance(target) / 1600).UnitPosition;
            for (int i = 2; i < 8; i++)
            {
    ...
    internal class PassiveManager
    {
        ...
        public List<Vector3> getPositions()
        {
            ...
            var predEnemy = LeagueSharp.Common.Prediction.GetPrediction(Enemy, ObjectManager.Player.Distance(Enemy) / 1600).UnitPosition;
            foreach (var passive in passives)
            {
                switch (passive.Type)
                {
