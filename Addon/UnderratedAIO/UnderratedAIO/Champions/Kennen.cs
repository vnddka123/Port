...
namespace UnderratedAIO.Champions
{
    internal class Kennen
    {
            ...
            if (R.IsReady() && !player.HasBuffOfType(BuffType.Snare) &&
                ...
                (config.Item("userLow", true).GetValue<Slider>().Value <=
                 HeroManager.Enemies.Count(
                     e =>
                         e.IsValidTargetLS(config.Item("userrange", true).GetValue<Slider>().Value) &&
                         e.HealthPercent < 75)))
            {
                R.Cast();
            }
        ...
        private AIHeroClient getTarget()
        {
            ...
            {
                case 0:
                    return LeagueSharp.Common.TargetSelector.GetTarget(Q.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
                    break;
                case 1:
                    return LeagueSharp.Common.TargetSelector.GetTarget(Q.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
                    break;
                default:
                    return LeagueSharp.Common.TargetSelector.GetTarget(Q.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
                    break;
