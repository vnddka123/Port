    ...
    internal class Gangplank
    {
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            var barrels =
                GetBarrels()
                    ...
                    .ToList();
            var QMana = Q.Instance.SData.Mana < player.Mana;
                ...
                foreach (var enemy in
                    HeroManager.Enemies.Where(
                        e =>
                            ...
                            e.IsValidTargetLS() && e.Distance(player) > 1500))
                {
            ...
            if (config.Item("EQtoCursor", true).GetValue<KeyBind>().Active && E.IsReady() && Q.IsReady())
            {
                ...
                if (barrel != null)
                {
                    ...
                    var cursorPos = barrel.Distance(cp) > BarrelConnectionRange
                        ? barrel.Position.Extend(cp, BarrelConnectionRange).To3DWorld()
                        : cp;
                    var points =
                    ...
                    var cursorPos2 = cursorPos.Distance(cp) > BarrelConnectionRange
                        ? cursorPos.Extend(cp, BarrelConnectionRange).To3DWorld()
                        : cp;
            ...
            if (config.Item("QbarrelCursor", true).GetValue<KeyBind>().Active)
            {
                ...
                if (meleeRangeBarrel != null && EloBuddy.SDK.Orbwalker.CanAutoAttack && Q.IsReady())
                {
            ...
            if (NeedToBeDestroyed != null && NeedToBeDestroyed.IsValidTargetLS() && EloBuddy.SDK.Orbwalker.CanAutoAttack &&
            ...
            {
                var target = LeagueSharp.Common.TargetSelector.GetTarget(
                    E.Range + BarrelExplosionRange / 2f, LeagueSharp.Common.TargetSelector.DamageType.Physical, true,
        ...
        private void Harass(List<Obj_AI_Minion> barrels, bool shouldAAbarrel, bool qMana)
        {
            ...
            AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(
                E.Range + BarrelExplosionRange / 2f, LeagueSharp.Common.TargetSelector.DamageType.Physical);
        ...
        {
            var pred = LeagueSharp.Common.Prediction.GetPrediction(target, delay * 0.75f);
            if (pred.UnitPosition.Distance(barrel) < BarrelExplosionRange &&
                HeroManager.Enemies.Count(
                    enemy => enemy.IsValidTargetLS() && enemy.Distance(barrel) < BarrelExplosionRange) > 0)
            {
                return true;
            }
            return false;
        ...
        private Vector3 GetE(Vector3 barrel, AIHeroClient target, float delay, List<Vector3> barrels)
        {
            var enemies =
                HeroManager.Enemies.Where(
                    e =>
                        e.IsValidTargetLS(1650) && e.Distance(barrel) > BarrelExplosionRange &&
        ...
        private void CastE(AIHeroClient target, List<Obj_AI_Minion> barrels)
        {
            ...
            var enemies =
                HeroManager.Enemies.Where(e => e.IsValidTargetLS() && e.Distance(player) < E.Range)
        ...
        private void Game_OnDraw(EventArgs args)
        {
            ...
            if (player.InFountainLS() && config.Item("drawQpass", true).GetValue<bool>() && tokens != null &&
                tokens.Count > 500)
            {
            ...
            if (config.Item("drawKillableSL", true).GetValue<StringList>().SelectedIndex != 0 && R.IsReady())
            {
                ...
                foreach (var enemy in HeroManager.Enemies.Where(e => e.IsValidTargetLS()))
                {
            ...
            try
            {
                if (Q.IsReady() && config.Item("drawEQ", true).GetValue<bool>())
                {
                    ...
                    if (barrel != null)
                    {
                        ...
                        {
                            Render.Circle.DrawCircle(
                                middle.Extend(cp, BarrelConnectionRange).To3DWorld(), BarrelExplosionRange, Color.DarkOrange, 6);
                            Render.Circle.DrawCircle(middle, BarrelExplosionRange, Color.DarkOrange, 6);
                            Drawing.DrawLine(
                                Drawing.WorldToScreen(barrel.Position),
                                Drawing.WorldToScreen(middle.Extend(barrel.Position, BarrelExplosionRange).To3DWorld()), 2,
                                Color.DarkOrange);
                        }
                        else if (GetAmmo() > 0)
                        {
                            Drawing.DrawLine(
                                Drawing.WorldToScreen(barrel.Position),
                                Drawing.WorldToScreen(cursorPos.Extend(barrel.Position, BarrelExplosionRange).To3DWorld()), 2,
                                Color.DarkOrange);
            ...
            if (sender.IsMe)
            {
                if (args.SData.Name == "GangplankQWrapper")
                {
                    ...
                    if (targetBarrel != null && targetBarrel.Distance(player) > Q.Range * 0.985f)
                    {
                        var enemy =
                            HeroManager.Enemies.Where(
                                e => e.IsValidTargetLS() && !GetBarrels().Any(b => b.Distance(e) < BarrelExplosionRange))
                                .OrderBy(e => e.Health)
                                .FirstOrDefault();

                        if (enemy != null && E.IsReady())
                        {
                            var pred = LeagueSharp.Common.Prediction.GetPrediction(enemy, 0.55f).CastPosition;
                            if (pred.Distance(player.Position) < E.Range)
                            {
                                thirdEpos = targetBarrel.Position.Extend(pred, BarrelConnectionRange).To3DWorld();
                                LeagueSharp.Common.Utility.DelayAction.Add(500, () => thirdEpos = Vector3.Zero);
                            }
                        }
                    }
