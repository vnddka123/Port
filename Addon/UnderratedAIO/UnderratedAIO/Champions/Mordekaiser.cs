    ...
    internal class Mordekaiser
    {
        ...
        private void CastW(AIHeroClient target)
        {
            ...
            if (allyWs.Any())
            {
                ...
                {
                    var targethero =
                        HeroManager.Enemies.Where(e => e.Distance(allyW.Position) < wWidth && e.IsValidTargetLS())
                            .OrderByDescending(e => e.Distance(allyW.Position))
                            .FirstOrDefault();
                    if (targethero != null)
                    {
                        var pred =
                            LeagueSharp.Common.Prediction.GetPrediction(targethero, 0.1f)
                                .UnitPosition.Distance(LeagueSharp.Common.Prediction.GetPrediction(allyW, 0.1f).UnitPosition);
                        if ((pred > wWidth &&
                             (allyW.CountEnemiesInRange(wWidth) == 1 || target.NetworkId == targethero.NetworkId)) &&
                            !allyWs.Any(
                                a =>
                                    LeagueSharp.Common.Prediction.GetPrediction(targethero, 0.1f)
                                        .UnitPosition.Distance(LeagueSharp.Common.Prediction.GetPrediction(a, 0.1f).UnitPosition) < wWidth))
                        {
                            W.Cast();
                        }
                    }
