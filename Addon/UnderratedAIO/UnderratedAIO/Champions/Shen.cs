...
namespace UnderratedAIO.Champions
{
    internal class Shen
    {
        ...
        private void OnPossibleToInterrupt(AIHeroClient unit, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            if (unit.IsValidTargetLS(E.Range) && E.IsReady())
            {
                E.Cast(unit);
            }
        }
        ...
        private static void OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (gapcloser.Sender.IsValidTargetLS(E.Range) && E.IsReady() &&
        ...
        {
            var minionsHP = ObjectManager.Get<Obj_AI_Minion>().Where(m => m.IsValidTargetLS(400)).Sum(m => m.Health);
            ...
            if (Program.IsSPrediction)
            {
                ...
                var enemiesBehind =
                    HeroManager.Enemies.Count(
                        e =>
                            e.NetworkId != target.NetworkId && e.IsValidTargetLS(E.Range) &&
        ...
        public static void CastEmin(AIHeroClient target, int min)
        {
            ...
            else
            {
                ...
                foreach (var enemy in
                    ObjectManager.Get<AIHeroClient>()
                        .Where(i => i.Distance(player) < E.Range && i.IsEnemy && !i.IsDead && i.IsValidTargetLS()))
                {
        ...
        {
            AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(EFlash.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
            if (target != null && E.IsReady() && E.Instance.SData.Mana < player.Mana &&
