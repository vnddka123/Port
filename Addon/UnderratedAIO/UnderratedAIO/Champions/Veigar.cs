...
namespace UnderratedAIO.Champions
{
    internal class Veigar
    {
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            switch (orbwalker.ActiveMode)
            {
                case Orbwalking.OrbwalkingMode.Combo:
                    AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(
                        1000, LeagueSharp.Common.TargetSelector.DamageType.Magical, true, HeroManager.Enemies.Where(h => h.IsInvulnerable));
                    if (target != null)
                    {
            ...
            if (config.Item("autoW", true).GetValue<bool>() || config.Item("autoE", true).GetValue<bool>())
            {
                targ =
                    HeroManager.Enemies.Where(
                        ...
                        .OrderByDescending(hero => LeagueSharp.Common.TargetSelector.GetPriority(hero))
            ...
            {
                var enemyQ =
                    HeroManager.Enemies.Where(e => e.Health < Q.GetDamage(e) && e.IsValidTargetLS() && Q.CanCast(e))
            ...
            if (R.IsReady() && config.Item("ksR", true).GetValue<bool>())
            {
                var enemyR =
                    HeroManager.Enemies.Where(
                        e =>
                            R.CanCast(e) && e.Health < R.GetDamage(e) && e.IsValidTargetLS() &&
        ...
        private void Combo(AIHeroClient target, float cmbDmg, bool canKill, bool bush)
        {
            ...
            if (config.Item("useq", true).GetValue<bool>() && Q.IsReady() && Q.CanCast(target) && target.IsValidTargetLS() &&
                !bush && !Estun)
            {
                CastQHero(target);
            }
                ...
                !CombatHelper.CheckCriticalBuffs(target))
            {
                if (config.Item("userPred", true).GetValue<bool>())
                {
                    var Whit = wPos.IsValid() && System.Environment.TickCount - wTime > 700 &&
                               LeagueSharp.Common.Prediction.GetPrediction(target, 0.55f).UnitPosition.Distance(wPos) < W.Width;
        ...
        private void CastE(AIHeroClient target, bool edge = true, int minHits = 1)
        {
            ...
            else
            {
                if (player.CountEnemiesInRange(E.Range + 175) <= 1)
                {
                    ...
                    {
                        E.Cast(edge ? pos.Extend(player.Position, 375).To3DWorld() : pos);
                    }
                }
                else
                {
                ...
                var objAiBases = from minion in minions
                    let pred =
                        Q.GetCollision(
                            ...
                            new List<Vector2>() { player.Position.Extend(minion.Position, Q.Range).To3DWorld().To2D() }, 70f)
        ...
        private Vector3 getBestEVector3(AIHeroClient target, int minHits = 1)
        {
            ...
            var otherHeroes =
                HeroManager.Enemies.Where(
                    e => e.IsValidTargetLS() && e.NetworkId != target.NetworkId && player.Distance(e) < 1000)
