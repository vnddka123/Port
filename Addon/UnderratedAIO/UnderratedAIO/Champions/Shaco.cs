    ...
    internal class Shaco
    {
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(
                Q.Range + player.MoveSpeed * 3, LeagueSharp.Common.TargetSelector.DamageType.Physical);
            ...
            if (E.IsReady())
            {
                var ksTarget =
                    HeroManager.Enemies.FirstOrDefault(
                        h => h.IsValidTargetLS() && !CombatHelper.IsInvulnerable2(h) && h.Health < E.GetDamage(h));
                if (ksTarget != null)
                {
        ...
        private void Combo(AIHeroClient target)
        {
                 ...
                 (cmbDmg > target.Health && player.CountEnemiesInRange(2000) == 1)))
            {
                ...
                {
                    Q.Cast(LeagueSharp.Common.Prediction.GetPrediction(target, 0.5f).UnitPosition);
                }
                else
                {
        ...
        private void HandleW(AIHeroClient target)
        {
            ...
            else
            {
                if (target.IsMoving)
                {
                    var pred = LeagueSharp.Common.Prediction.GetPrediction(target, 2);
        ...
        private void CastW(AIHeroClient target, Vector3 from, Vector3 to)
        {
            ...
            for (int i = 1; i < 11; i++)
            {
                positions.Add(from.Extend(to, 42 * i).To3DWorld());
            }
            var best =
        ...
        private void Harass()
        {
            AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(E.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
            if (target == null)
            {
                return;
            }
