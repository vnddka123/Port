...
namespace UnderratedAIO.Champions
{
    internal class Trundle
    {
        ...
        {
            AIHeroClient targetO = LeagueSharp.Common.TargetSelector.GetTarget(E.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical, true);
        ...
        private void Clear()
        {
            ...
            if (config.Item("useqLC", true).GetValue<bool>() && Q.IsReady() && !EloBuddy.SDK.Orbwalker.CanAutoAttack)
            {
                Q.Cast();
            }
        ...
        private void Combo()
        {
            AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(E.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical, true);
            ...
            {
                var pos = player.Position.Extend(LeagueSharp.Common.Prediction.GetPrediction(target, 700).UnitPosition, W.Range / 2);
            ...
            if (config.Item("user", true).GetValue<bool>() && R.IsReady())
            {
                ...
                switch (config.Item("userTarget", true).GetValue<StringList>().SelectedIndex)
                {
                    case 0:
                        targetR =
                            HeroManager.Enemies.Where(e => e.IsValidTargetLS(R.Range))
                                .OrderByDescending(e => (e.Armor + e.FlatMagicReduction))
                                .FirstOrDefault();
                        break;
                    case 1:
                        targetR = target.IsValidTargetLS(R.Range) ? target : null;
                        break;
                }
        ...
        private Vector3 GetVectorE(AIHeroClient target)
        {
            ...
            var pred = LeagueSharp.Common.Prediction.GetPrediction(target, 0.28f);
            if (!target.IsMoving)
            {
                return pos;
            }
            ...
            {
                pos = wall.Extend(pred.UnitPosition, (target.BoundingRadius + distW)).To3DWorld();
            }
            ...
            if (pred.UnitPosition.Distance(player.Position) > player.Distance(target))
            {
                ...
                {
                    predPos = target.Position.Extend(predPos, dist).To3DWorld();
                }
                pos = predPos.Extend(target.Position, -dist).To3DWorld();
            }
            return pos;
        }
