    ...
    internal class Galio
    {
        ...
        private void Combo()
        {
            AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(
                E.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical, true, HeroManager.Enemies.Where(h => h.IsInvulnerable));
            if (target == null)
            {
                return;
            }
        ...
        private void CastW(bool combo)
        {
            foreach (var h in
                HeroManager.Allies.Where(i => i.IsValid && i.Distance(player) < W.Range)
                    .OrderByDescending(i => LeagueSharp.Common.TargetSelector.GetPriority(i)))
            {
