...
namespace UnderratedAIO.Champions
{
    internal class Jax
    {
        ...
        private void WardJump()
        {
            ...
            {
                pos = player.Position.Extend(pos, 600).To3DWorld();
            }
        ...
        public Obj_AI_Base GetJumpObj(Vector3 pos)
        {
            return
                ObjectManager.Get<Obj_AI_Base>()
                    .Where(
                        obj =>
                            obj.IsValidTargetLS(600, false) && pos.Distance(obj.ServerPosition) <= 100 &&
        ...
        private void Combo()
        {
            ...
            if (config.Item("useIgnite", true).GetValue<bool>() && ignitedmg > target.Health && hasIgnite &&
                ...
                ((target.Distance(player) > Orbwalking.GetRealAutoAttackRange(target) &&
                  (!Q.IsReady() || Q.Instance.SData.Mana < player.Mana)) || player.HealthPercent < 35))
            {
            ...
            if (Eactive)
            {
                if (E.IsReady() && target.IsValidTargetLS() && !target.MagicImmune &&
                    ((LeagueSharp.Common.Prediction.GetPrediction(target, 0.1f).UnitPosition.Distance(player.Position) >
