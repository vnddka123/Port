...
namespace UnderratedAIO.Champions
{
    internal class Sion
    {
        ...
        private void Harass()
        {
            ...
            AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(1500, LeagueSharp.Common.TargetSelector.DamageType.Physical, true);
        ...
        private void Combo()
        {
            ...
            {
                var rTarget = LeagueSharp.Common.TargetSelector.GetTarget(2500, LeagueSharp.Common.TargetSelector.DamageType.Physical, true);
                        ...
                        if (player.Distance(rTarget) + 100 > Environment.Map.GetPath(player, rTarget.Position) &&
                            (ComboDamage(rTarget) > rTarget.Health &&
                             !CombatHelper.IsCollidingWith(
                                 player, rTarget.Position.Extend(player.Position, player.BoundingRadius + 15).To3DWorld(),
            ...
            if (activatedW && config.Item("usew", true).GetValue<bool>() && W.IsReady() &&
                ...
                (target.Health < W.GetDamage(target) ||
                 (W.IsInRange(target) && !W.IsInRange(LeagueSharp.Common.Prediction.GetPrediction(target, 0.2f).UnitPosition))))
            {
                W.Cast();
            }
        ...
        {
            if ((justQ && target.Distance(player) > Q.Range) || !target.IsValidTargetLS())
            {
                return;
            }
            var poly = GetPoly(lastQPos);
            var heroes = HeroManager.Enemies.Where(e => poly.IsInside(e.Position) && e.IsValidTargetLS());
            var objAiHeroes = heroes as IList<AIHeroClient> ?? heroes.ToList();
            if (objAiHeroes.Any())
            {
                var escaping =
                    objAiHeroes.Count(h => poly.IsOutside(LeagueSharp.Common.Prediction.GetPrediction(h, 0.2f).UnitPosition.To2D()));
                var data = Program.IncDamages.GetAllyData(player.NetworkId);
                if ((escaping > 0 &&
                     (objAiHeroes.Count() == 1 ||
        ...
        private void CastEHero(AIHeroClient target)
        {
            ...
            var pred = LeagueSharp.Common.Prediction.GetPrediction(
            ...
            if (collision.Any(c => c.Distance(player) < E.Range) &&
                !CombatHelper.IsCollidingWith(
                    player, pred.CastPosition.Extend(player.Position, W.Width + 15).To3DWorld(), E.Width,
                    new[] { CollisionableObjects.Heroes, CollisionableObjects.Walls }))
            {
            ...
            if (sender.IsMe)
            {
                if (args.SData.Name == "SionQ")
                {
                    if (!justQ)
                    {
                        ...
                        lastQPos = player.Position.Extend(args.End, Q.Range).To3DWorld();
