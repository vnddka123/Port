            ...
            else
            {
                     ...
                     config.Item("userEnabled", true).GetValue<bool>()) || manual)
                {
                    if (target.IsMoving)
                    {
                        ...
                        {
                            R.Cast(target.Position.Extend(pos, -500).To3DWorld(), pos);
                        }
                    }
                    else
                    {
                        R.Cast(target.Position.Extend(player.Position, 500).To3DWorld(), target.Position);
                    }
                }
                else if (targE.Hitchance >= HitChance.High && config.Item("userEnabled", true).GetValue<bool>())
                {
                    ...
                    if (pred != Vector3.Zero &&
                        CombatHelper.GetCollisionCount(
                            target, target.Position.Extend(pred, 1000).To3DWorld(), R.Width, new[] { CollisionableObjects.Heroes, }) >=
                        config.Item("Rmin", true).GetValue<Slider>().Value)
                    {
                        R.Cast(target.Position.Extend(pred, -target.MoveSpeed).To3DWorld(), pred);
                    }
                }
            }
        }
            ...
            if (otherHeroes.Any())
            {
                var best =
                    otherHeroes.OrderByDescending(
                        hero =>
                            CombatHelper.GetCollisionCount(
                                target, target.Position.Extend(hero.CastPosition, 1000).To3DWorld(), R.Width,
