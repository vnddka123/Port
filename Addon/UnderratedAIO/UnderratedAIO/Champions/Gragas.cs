...
namespace UnderratedAIO.Champions
{
    internal class Gragas
    {
        ...
        public static bool justQ, useIgnite, justE, justR;
        ...
        private void OnInterruptableTarget(AIHeroClient target, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            if (R.CanCast(target) && config.Item("useRint", true).GetValue<bool>())
            {
                ...
                {
                    var cast = LeagueSharp.Common.Prediction.GetPrediction(target, 1000f).UnitPosition.Extend(savedQ.position, -100);
                    R.Cast(cast);
                }
        ...
        private void DetonateQ()
        {
            ...
            var targethero =
                HeroManager.Enemies.Where(e => e.Distance(savedQ.position) < QExplosionRange && e.IsValidTargetLS())
            ...
            if (savedQ.deltaT() < 2000 &&
                LeagueSharp.Common.Prediction.GetPrediction(targethero, 0.1f).UnitPosition.Distance(savedQ.position) < QExplosionRange &&
                HeroManager.Enemies.Count(
                    h => h.Distance(savedQ.position) < QExplosionRange && h.IsValidTargetLS() && h.Health < getQdamage(h)) ==
                0)
            {
                //waiting
            }
        ...
        private bool checkMana()
        {
            ...
            if (Q.IsReady())
            {
                manareq += Q.Instance.SData.Mana;
            }
            if (W.IsReady())
            {
                manareq += W.Instance.SData.Mana;
            }
            if (E.IsReady())
            {
                manareq += E.Instance.SData.Mana;
            }
            if (R.IsReady())
            {
                manareq += R.Instance.SData.Mana;
            }
        ...
        private bool CheckRPushForAlly(AIHeroClient target, float combodmg)
        {
            ...
            var turret =
                ObjectManager.Get<Obj_AI_Turret>()
                    .OrderBy(t => t.Distance(target))
                    .FirstOrDefault(t => t.Distance(target) < 2000 && t.IsEnemy && t.IsValidTargetLS());
