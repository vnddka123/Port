...
namespace UnderratedAIO.Champions
{
    internal class Chogath
    {
        ...
        public static LeagueSharp.Common.Spell Q, W, E, R, RFlash;
        ...
        {
            var minions = ObjectManager.Get<Obj_AI_Minion>().Where(m => m.IsValidTargetLS(400)).ToList();
            if (minions.Count() > 2)
            {
        ...
        private static void Harass()
        {
            ...
            {
                if (target.IsValidTargetLS(Q.Range) && Q.IsReady())
                {
                    Q.Cast(target);
                }
            }
            ...
            {
                if (target.IsValidTargetLS(W.Range) && W.IsReady())
                {
                    W.Cast(target);
                }
            }
        ...
        private static void OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            {
                if (gapcloser.Sender.IsValidTargetLS(Q.Range) && Q.IsReady())
                {
            ...
            {
                if (gapcloser.Sender.IsValidTargetLS(W.Range) && W.IsReady())
                {
