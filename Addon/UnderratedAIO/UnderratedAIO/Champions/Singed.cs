...
namespace UnderratedAIO.Champions
{
    internal class Singed
    {
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            {
                EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, cgTarg.Position.Extend(forcedPos, 50).To3DWorld());
            }
            if (forcedPos.IsValid() && ToCursor)
            {
                var pos = cgTarg.Position.Extend(forcedPos, 115).To3DWorld();
        ...
        private void Throw()
        {
            AIHeroClient target = LeagueSharp.Common.TargetSelector.GetTarget(W.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
            if (target == null)
            {
                return;
            }
        ...
        private void Combo()
        {
                ...
                if (target.IsMoving)
                {
                    ...
                    if (ChaseFix)
                    {
                        pos = target.Position.Extend(
                            LeagueSharp.Common.Prediction.GetPrediction(target, 0.6f).UnitPosition, rand.Next(480, 550)).To3DWorld();
                    }
                    else
                    {
            ...
            var qTarget =
                HeroManager.Enemies.FirstOrDefault(
                    enemy =>
                        enemy.IsValidTargetLS() && enemy.Distance(player) < 200 &&
            ...
            {
                var pos = LeagueSharp.Common.Prediction.GetPrediction(target, W.Delay / 2)
