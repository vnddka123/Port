...
namespace UnderratedAIO.Champions
{
    internal class Zac
    {
        ...
        private void Combo()
        {
            ...
            if (E.IsCharging)
            {
                target = LeagueSharp.Common.TargetSelector.GetTarget(
                    GetTargetRange(), LeagueSharp.Common.TargetSelector.DamageType.Magical, true,
            ...
            if (config.Item("useq", true).GetValue<bool>() && Q.CanCast(target) && target.IsValidTargetLS() &&
                !E.IsCharging)
            {
                Q.CastIfHitchanceEquals(target, HitChance.Medium);
            }
        ...
        private void CastE(Obj_AI_Base target)
        {
            ...
            var enemyPred = LeagueSharp.Common.Prediction.GetPrediction(
