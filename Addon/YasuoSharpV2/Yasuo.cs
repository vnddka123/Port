...
namespace YasuoSharpV2
{
    class Yasuo
    {
        ...
        public static Spell Q = new Spell(SpellSlot.Q, 450);
...
//        public static List<string> WIgnore;
        ...
        {
            List<Obj_AI_Base> jumps = ObjectManager.Get<Obj_AI_Base>().Where(enemy => enemyIsJumpable(enemy) && enemy.IsValidTargetLS(550, true, dash.to)).ToList();
...
//                Obj_AI_Base bestos = null;
        ...
        public static void stackQ()
        {
            ...
            if (!isDashigPro)
            {
                ...
                Vector2 clos = LeagueSharp.Common.Geometry.ClosestLS(Player.ServerPosition.To2D(), minionPs);
...
//                    SmoothMouse.addMouseEvent(clos.To3D());
        ...
        public static Obj_AI_Base canDoEQEasly(AIHeroClient target)
        {
            if (!E.IsReady() || Q.IsReady(150) || !isQEmpovered())
                return null;
            List<Obj_AI_Base> jumps = ObjectManager.Get<Obj_AI_Base>().Where(enemy => enemy.NetworkId != target.NetworkId && enemyIsJumpable(enemy) && enemy.IsValidTargetLS(470, true)).OrderBy(jp => V2E(Player.Position, jp.Position, 475).Distance(target.Position, true)).ToList();
...
//                    SmoothMouse.addMouseEvent(po.UnitPosition);
        ...
        {
            var minions = MinionManager.GetMinions(ObjectManager.Player.ServerPosition, Q.Range + 50);
            foreach (var minion in minions.Where(minion => minion.IsValidTargetLS(Q.Range)))
            {
...
//                            SmoothMouse.addMouseEvent(minion.Position);
        ...
        public static void doLaneClear(AIHeroClient target)
        {
            ...
            if (YasuoSharp.Config.Item("useElc").GetValue<bool>() && E.IsReady())
                foreach (var minion in minions.Where(minion => minion.IsValidTargetLS(E.Range) && enemyIsJumpable(minion)))
                {
...
//                        SmoothMouse.addMouseEvent(po.CastPosition);
        ...
        public static IsSafeResult isSafePoint(Vector2 point, bool igonre = false)
        {
            ...
            if (false)
            {
                bool safe = YasuoSharp.Orbwalker.ActiveMode.ToString() == "Combo" ||
                            point.To3D().GetEnemiesInRangeLS(500).Count > Player.HealthPercent%65;
                if (!safe)
                {
                    result.IsSafe = false;
                    return result;
        ...
        public static void useWSmart(Skillshot skillShot)
        {
            ...
            if (skillShot.IsAboutToHit(500, Player))
            {
                ...
                float spellDamage = (float)skillShot.Unit.GetSpellDamageLS(Player, sd.SpellName);
        ...
        public static void useWSmartOld(MissileClient missle)
        {
            ...
            try
            {
                if (missle.SpellCaster is AIHeroClient && missle.IsEnemy)
                {
                    ...
                    //Console.WriteLine("dmg: " + missle.SpellCaster.GetSpellDamage(Player, sd.SpellName));
                    float spellDamage = (float)missle.SpellCaster.GetSpellDamageLS(Player, sd.SpellName);
                    int procHp = (int)((spellDamage / Player.MaxHealth) * 100);
...
//                            SmoothMouse.addMouseEvent(blockwhere);
                                 ...
                                 !goesThroughWall(Player.Position, targMis.particle.Position))
                        {
                            ...
                            {
                                if (goesThroughWall(Player.Position, Player.Position.Extend(enemy.Position, 475).To3DWorld()))
                                {
                                    E.CastOnUnit(enemy);
                                    return;
                                }
