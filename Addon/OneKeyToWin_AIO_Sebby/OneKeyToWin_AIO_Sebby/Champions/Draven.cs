...
namespace OneKeyToWin_AIO_Sebby.Champions
{
    class Draven
    {
        ...
        private void LogicR()
        {
            ...
            {
                foreach (var target in HeroManager.Enemies.Where(target => target.IsValidTargetLS(R.Range) && OktwCommon.ValidUlt(target) && target.CountAlliesInRange(500) == 0))
                {
                    ...
                    else if (Config.Item("Rcc", true).GetValue<bool>() && Rdmg * 2 > predictedHealth && !OktwCommon.CanMove(target) &&  target.IsValidTargetLS( E.Range))
                    {
                        R.Cast(target);
                        Program.debug("R normal");
                    }
                    ...
                    else if (target.IsValidTargetLS(E.Range) && Rdmg * 2 + qDmg + eDmg > predictedHealth && Config.Item("Raoe", true).GetValue<bool>())
                    {
        ...
        private void AxeLogic()
        {
            ...
            if (Config.Item("axeKill", true).GetValue<bool>() && t.IsValidTargetLS() && Player.Distance(t.Position) > 400 && Player.GetAutoAttackDamage(t) * 2 > t.Health)
            {
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            if (Config.Item("qAxePos", true).GetValue<bool>())
            {
                ...
                foreach (var obj in axeList)
                {
                    ...
                    {
                        Render.Circle.DrawCircle(obj.Position, 150, System.Drawing.Color.OrangeRed, 1);
                    }
                    else if (Player.Distance(obj.Position) > 120)
                    {
                        Render.Circle.DrawCircle(obj.Position, 150, System.Drawing.Color.Yellow, 1);
                    }
                    else if (Player.Distance(obj.Position) < 150)
                    {
                        Render.Circle.DrawCircle(obj.Position, 150, System.Drawing.Color.YellowGreen, 1);
                    }
                }
            }

            if (Config.Item("qCatchRange", true).GetValue<bool>())
                Render.Circle.DrawCircle(Game.CursorPos, axeCatchRange, System.Drawing.Color.LightSteelBlue, 1);
