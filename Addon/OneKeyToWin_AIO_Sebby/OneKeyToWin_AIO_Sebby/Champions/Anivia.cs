...
//        private float RCastTime = 0;
        ...
        {
            if (Config.Item("inter", true).GetValue<bool>() && W.IsReady() && sender.IsValidTargetLS(W.Range))
                W.Cast(sender);
        }
        ...
        private void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            {
                if (Target.IsValidTargetLS(W.Range))
                {
        ...
        private void farmE()
        {
            if (Program.LaneClear && Config.Item("farmE", true).GetValue<bool>() && Player.Mana > QMANA + EMANA + WMANA && !EloBuddy.SDK.Orbwalker.CanAutoAttack /*Orbwalking.CanAttack*/ && Player.ManaPercent > Config.Item("Mana", true).GetValue<Slider>().Value)
            {
