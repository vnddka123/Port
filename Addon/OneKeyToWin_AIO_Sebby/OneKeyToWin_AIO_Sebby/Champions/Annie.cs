...
namespace OneKeyToWin_AIO_Sebby
{
    class Annie
    {
        ...
        private void farm()
        {
            ...
            if (Q.IsReady())
            {
                var minion = minionsList.Where(x => LeagueSharp.Common.HealthPrediction.LaneClearHealthPrediction(x, 250, 50) < Q.GetDamage(x) && x.Health > Player.GetAutoAttackDamage(x)).FirstOrDefault();
                Q.Cast(minion);
            }
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Config.Item("rRange", true).GetValue<bool>())
            {
                ...
                {
                    if (R.IsReady())
                        Render.Circle.DrawCircle(ObjectManager.Player.Position, R.Range + R.Width / 2, System.Drawing.Color.Gray, 1);
                }
                else
                    Render.Circle.DrawCircle(ObjectManager.Player.Position, R.Range + R.Width / 2, System.Drawing.Color.Gray, 1);
            }
