    ...
    internal class MoonDraven
    {
        ...
        private void AntiGapcloserOnOnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            if (!this.Menu.Item("UseEGapcloser").IsActive() || !this.E.IsReady()
                || !gapcloser.Sender.IsValidTargetLS(this.E.Range))
            {
                return;
            }
        ...
        private void Combo()
        {
            var target = LeagueSharp.Common.TargetSelector.GetTarget(this.E.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
            ...
            var killableTarget =
                HeroManager.Enemies.Where(x => x.IsValidTargetLS(2000))
                    .FirstOrDefault(
                        x =>
        ...
        private void CreateMenu()
        {
            ...
            LeagueSharp.Common.TargetSelector.AddToMenu(tsMenu);
        ...
        {
            if (!this.Menu.Item("UseEInterrupt").IsActive() || !this.E.IsReady() || !sender.IsValidTargetLS(this.E.Range))
            {
                return;
            }
