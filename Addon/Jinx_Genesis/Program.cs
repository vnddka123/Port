...
//        public static EloBuddy.SDK.Spell.Active Q;

        /// <summary>
        /// The Spells W, E, and R
        /// </summary>
//        public static EloBuddy.SDK.Spell.Skillshot W, E, R;
...
/*
            Q = new EloBuddy.SDK.Spell.Active(SpellSlot.Q);
            W = new EloBuddy.SDK.Spell.Skillshot(SpellSlot.W, 1500, SkillShotType.Linear, 500, 3300, 60)
            {
                AllowedCollisionCount = 0
            };
            E = new EloBuddy.SDK.Spell.Skillshot(SpellSlot.E, 900, SkillShotType.Circular, 250, 1750, 315)
            {
                AllowedCollisionCount = int.MaxValue
            };
            R = new EloBuddy.SDK.Spell.Skillshot(SpellSlot.R, 25000, SkillShotType.Linear, 500, 1700, 140)
            {
                AllowedCollisionCount = int.MaxValue
            };
//*/
        ...
        private static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (E.IsReady())
            {
                ...
                if (t.IsValidTargetLS(E.Range) && Config.Item("EGCchampion" + t.ChampionName).GetValue<bool>())
                {
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (!E.IsReady() || !sender.IsEnemy || !Config.Item("Espell").GetValue<bool>() || Player.ManaPercent < Config.Item("EmanaCombo").GetValue<Slider>().Value || !sender.IsValid<AIHeroClient>() || !sender.IsValidTargetLS(E.Range) )
                return;
        ...
        private static void Rlogic()
        {
            ...
            {
                var t = LeagueSharp.Common.TargetSelector.GetTarget(R.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
                if (t.IsValidTargetLS() && t.IsVisible && t.IsHPBarRendered && !t.IsDead)
                {
            ...
            if (Config.Item("Rks").GetValue<bool>())
            {
                ...
                if (Config.Item("RoverAA").GetValue<bool>() && (!EloBuddy.SDK.Orbwalker.CanAutoAttack /*Orbwalking.CanAttack*/ || Player.Spellbook.IsAutoAttacking))
                    return;

                foreach (var target in Enemies.Where(target => target.IsValidTargetLS(R.Range) && ValidUlt(target) && target.IsVisible && target.IsHPBarRendered))
                {
                    ...
                    if (Rdmg > predictedHealth)
                    {
                        ...
                        foreach (var enemy in Enemies.Where(enemy => enemy.IsValidTargetLS()))
                        {
                        ...
                        if (cast)
                        {
                            if (Config.Item("RoverW").GetValue<bool>() && target.IsValidTargetLS(W.Range) && W.GetDamage(target) > target.Health && W.Instance.Cooldown - (W.Instance.CooldownExpires - Game.Time) < 1.1)
                                return;
        ...
        private static bool RValidRange(Obj_AI_Base t)
        {
            ...
            {
                if (range > Config.Item("Rcustome").GetValue<Slider>().Value && !EloBuddy.Player.Instance.IsInAutoAttackRange(t))// Orbwalking.InAutoAttackRange(t))
                    return true;
                else
                    return false;
            }
        ...
        private static void Elogic()
        {
            ...
            {
                foreach (var enemy in Enemies.Where(enemy => enemy.IsValidTargetLS(2000) && enemy.HasBuff("RocketGrab")))
                {
            ...
            foreach (var enemy in Enemies.Where(enemy => enemy.IsValidTargetLS(E.Range) ))
            {
            ...
            {
                var t = LeagueSharp.Common.TargetSelector.GetTarget(E.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
        ...
        {
            var t = LeagueSharp.Common.TargetSelector.GetTarget(W.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
            if (t.IsValidTargetLS() && WValidRange(t))
            {
                ...
                else if (Farm && EloBuddy.SDK.Orbwalker.CanAutoAttack && !Player.Spellbook.IsAutoAttacking && Config.Item("Wharass").GetValue<bool>() && Player.ManaPercent > Config.Item("WmanaHarass").GetValue<Slider>().Value)
                {
                    ...
                    {
                        foreach (var enemy in Enemies.Where(enemy => enemy.IsValidTargetLS(W.Range) && WValidRange(t) && Config.Item("haras" + enemy.ChampionName).GetValue<bool>()))
                            CastSpell(W, enemy);
                    }
                }
        ...
        private static void Qlogic()
        {
            ...
            {
                var t = LeagueSharp.Common.TargetSelector.GetTarget(Q.Range + 40, LeagueSharp.Common.TargetSelector.DamageType.Physical);
                if (t.IsValidTargetLS())
                {
                    if ((!EloBuddy.Player.Instance.IsInAutoAttackRange(t) || t.CountEnemiesInRange(250) >= Config.Item("Qaoe").GetValue<Slider>().Value))
                    {
                        ...
                        if (Farm && EloBuddy.SDK.Orbwalker.CanAutoAttack && !Player.Spellbook.IsAutoAttacking && Config.Item("harasQ" + t.ChampionName).GetValue<bool>() && Config.Item("Qharass").GetValue<bool>() && (Player.ManaPercent > Config.Item("QmanaHarass").GetValue<Slider>().Value || Player.GetAutoAttackDamage(t) * Config.Item("QmanaIgnore").GetValue<Slider>().Value > t.Health))
                        {
                            Q.Cast();
                        }
                    }
                ...
                else
                {
                    ...
                    else if (Farm && !Player.Spellbook.IsAutoAttacking && Config.Item("farmQout").GetValue<bool>() && EloBuddy.SDK.Orbwalker.CanAutoAttack)
                    {
                        foreach (var minion in MinionManager.GetMinions(Q.Range + 30).Where(
                        minion => !EloBuddy.Player.Instance.IsInAutoAttackRange(minion) && minion.Health < Player.GetAutoAttackDamage(minion) * 1.2 && GetRealPowPowRange(minion) < GetRealDistance(minion) && Q.Range < GetRealDistance(minion)))
                        {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                if (!FishBoneActive)
                    Render.Circle.DrawCircle(Player.Position, 590f + Player.BoundingRadius, System.Drawing.Color.Gray, 1);
                else
                    Render.Circle.DrawCircle(Player.Position, Q.Range - 40, System.Drawing.Color.Gray, 1);
            }
            if (Config.Item("wRange").GetValue<bool>())
            {
                ...
                {
                    if (W.IsReady())
                        Render.Circle.DrawCircle(Player.Position, W.Range, System.Drawing.Color.Gray, 1);
                }
                else
                    Render.Circle.DrawCircle(Player.Position, W.Range, System.Drawing.Color.Gray, 1);
            }
            if (Config.Item("eRange").GetValue<bool>())
            {
                ...
                {
                    if (E.IsReady())
                        Render.Circle.DrawCircle(Player.Position, E.Range, System.Drawing.Color.Gray, 1);
                }
                else
                    Render.Circle.DrawCircle(Player.Position, E.Range, System.Drawing.Color.Gray, 1);
            }
