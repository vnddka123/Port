    ...
    class Program
    {
        ...
        private static void Rlogic()
        {
            ...
            if (Config.Item("useR").GetValue<KeyBind>().Active)
            {
                ...
                if (t.IsValidTarget() && t.IsVisible && t.IsHPBarRendered && !t.IsDead)
                {
            ...
            if (Config.Item("Rks").GetValue<bool>())
            {
                ...
                foreach (var target in Enemies.Where(target => target.IsValidTarget(R.Range) && ValidUlt(target) && target.IsVisible && target.IsHPBarRendered))
                {
