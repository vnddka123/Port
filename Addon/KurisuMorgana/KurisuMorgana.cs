...
namespace KurisuMorgana
{
    internal class KurisuMorgana
    {
        ...
        public KurisuMorgana()
        {
            ...
            kemenu.AddItem(new MenuItem("harasskey", ":: Harass [active]")).SetValue(new KeyBind('X', KeyBindType.Press));
            kemenu.AddItem(new MenuItem("farmkey", ":: WaveClear [active]")).SetValue(new KeyBind('V', KeyBindType.Press));
            kemenu.AddItem(new MenuItem("fleekey", ":: Flee [active]")) .SetValue(new KeyBind('C', KeyBindType.Press));
        ...
        private static void Orbwalking_BeforeAttack(Orbwalking.BeforeAttackEventArgs args)
        {
            if (_menu.Item("support").GetValue<bool>())
            {
                    ...
                    _orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LastHit)
                {
                    ...
                    if (minion != null && minion.IsMinion && minion.IsValidTargetLS())
                    {
                        if (HeroManager.Allies.Any(x => x.IsValidTargetLS(1000, false) && !x.IsMe))
                        {
                            args.Process = false;
                        }
                    }
        ...
        {
            if (!Me.IsValidTargetLS(300, false) || !Orbwalking.CanMove(100))
            {
                return;
            }
        ...
        private static void AutoCast(bool dashing, bool immobile, bool soil)
        {
            if (_q.IsReady())
            {
                foreach (var itarget in HeroManager.Enemies.Where(h => h.IsValidTargetLS(_q.Range)))
                {
            ...
            if (_w.IsReady() && soil)
            {
                foreach (var itarget in HeroManager.Enemies.Where(h => h.IsValidTargetLS(_w.Range)))
                    if (immobile && Immobile(itarget))
        ...
        {
            if (gapcloser.Sender.IsValidTargetLS(250f))
            {
        ...
        internal static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsEnemy && sender.Type == GameObjectType.AIHeroClient && _q.IsReady())
            {
                if (args.End.IsValid() && args.End.Distance(Me.ServerPosition) <= 200 + Me.BoundingRadius)
                {
                    ...
                    if (!hero.IsValid<AIHeroClient>() || !hero.IsValidTargetLS(_q.Range - 50))
                    {
                        return;
                    }
            ...
            foreach (var ally in HeroManager.Allies.Where(x => x.IsValidTargetLS(_e.Range, false)))
            {
