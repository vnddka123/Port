...
namespace KurisuNidalee
{
    internal static class KurisuLib
    {
        ...
        internal static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (attacker.IsValidTargetLS(275f) && !Player.UnderTurret(true))
            {
                if (CatForm())
                {
        ...
        /// <param name="unit"></param>
        /// <returns></returns>
        internal static bool IsHunted1(this Obj_AI_Base unit)
        {          
            return unit.HasBuff("NidaleePassiveHunted") || unit.HasBuff("exposeweaknessdebuff");
        }
            ...
            foreach (var minion in MinionManager.GetMinions(900f, MinionTypes.All, MinionTeam.Neutral))
            {
                ...
                if (LargeList.Any(name => minion.Name.StartsWith(name) && !minion.Name.Contains("Mini")))
                {
                    if (KN.Root.Item("jgsmitebg").GetValue<bool>())
                    {
                        ...
                        {
                            if (Player.GetSpellDamage(minion, SpellSlot.Q) + damage >= minion.Health)
                ...
                Player.GetSpell(Smite).Name.ToLower() == "s5_summonersmiteplayerganker")
            {
                    ...
                    foreach (
                        var hero in
                            HeroManager.Enemies.Where(
                                h =>
                                    h.IsValidTargetLS(500) && !h.IsZombie &&
                    ...
                    Player.GetSpell(Smite).Name.ToLower() == "s5_summonersmiteplayerganker")
                {
                    foreach (
                        var hero in
                            HeroManager.Enemies
                                .Where(h => h.IsValidTargetLS(500) && !h.IsZombie)
