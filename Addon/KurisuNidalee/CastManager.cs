    ...
    internal class CastManager
    {
        ...
        {           
            ...
            if (!KL.CatForm() && KL.CanUse(KL.Spells["Bushwhack"], true, mode))
            {
                ...
                if (mode != "ha" || KL.Player.ManaPercent > 65)
                {
                    if (target.IsValidTargetLS(KL.Spells["Bushwhack"].Range))
                    {
        ...
        internal static void CastTakedown(Obj_AI_Base target, string mode)
        {
            ...
            {
                if (target.IsValidTargetLS(KL.Player.AttackRange + KL.Spells["Takedown"].Range))
                {
        ...
        internal static void CastPounce(Obj_AI_Base target, string mode)
        {
            ...
            if (!target.IsValidTargetLS(KL.Spells["ExPounce"].Range))
                return;
        ...
        internal static void CastSwipe(Obj_AI_Base target, string mode)
        {
            ...
            {
                if (target.IsValidTargetLS(KL.Spells["Swipe"].Range))
                {
                    if (target.IsChampion())
                    {
            ...
            if (!KL.CatForm() && KL.CanUse(KL.Spells["Aspect"], true, mode))
            {
                switch (mode)
                {
                    ...
                    case "gap":
                        ...
                        break;
                    case "wc":
                        if (target.IsValidTargetLS(375) && target.IsMinion)
                        {
                            KL.Spells["Aspect"].Cast();
                            return;
                        }
                ...
                if (target.IsHunted())
                {
                    ...
                    if (KL.Spells["Aspect"].IsReady() && target.IsValidTargetLS(KL.Spells["ExPounce"].Range))
                    {
                ...
                else
                {
                    ...
                    if (target.IsValidTargetLS(KL.Spells["Pounce"].Range + 55))
                    {
                        if (mode != "jg")
                        {
                            ...
                            {
                                if (mode == "co" && target.IsValidTargetLS(KL.Spells["Pounce"].Range + 200))
                                {
                            ...
                            var spellQ = KL.Player.Spellbook.GetSpell(SpellSlot.Q);
                    ...
                    if (KN.Target.IsValidTargetLS(KL.Spells["Javelin"].Range) && target.IsChampion())
                    {
                        if (KL.SpellTimer["Javelin"].IsReady())
                        {
                            // check if in pounce range.
