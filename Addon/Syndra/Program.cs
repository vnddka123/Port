...
namespace Syndra
{
    ...
    internal static class Program
    {
        ...
        private static void Game_OnWndProc(WndEventArgs args)
        {
            ...
            if (EloBuddy.Player.Instance.InShop() || ObjectManager.Player.InFountainLS()) return;
        ...
        private static void InstantQe2Enemy()
        {
            ...
            if (t.IsValidTargetLS() && E.IsReady() && Q.IsReady())
            {
                UseQe(t);
            }
        }
        ...
        private static Vector3 GetGrabableObjectPos(bool onlyOrbs)
        {
            if (!onlyOrbs)
            {
                foreach (var minion in ObjectManager.Get<Obj_AI_Minion>().Where(minion => minion.IsValidTargetLS(W.Range)))
                {
                ...
                foreach (
                    var enemy in
                        HeroManager.Enemies
                            .Where(
                                enemy =>
                                enemy.IsValidTargetLS(Eq.Range) && Game.CursorPos.Distance(enemy.ServerPosition) < 300))
                {
                    UseQe(enemy);
                }
            }
