...
//        private static int qq;
        ...
        private static void TryIgnote(Obj_AI_Base target)
        {
            ...
            if (player.Spellbook.CanUseSpell(ignote) == SpellState.Ready)
            {
                if (target.Distance(player.ServerPosition) <= 600)
                {
                    ...
                    {
                        if (ComboDamage(target) >= target.Health && target.Health / target.MaxHealth * 100 > menuslide("overk") || orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Burst)
                        {
                            if (r.IsReady() && uo)
                            {
        ...
        public KurisuRiven()
        {
            ...
            r.SetSkillshot(0.25f, 90f, 1600f, false, SkillshotType.SkillshotCone);
        ...
        {
            // harass active
            didhs = orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Mixed;
...
//                        TryIgnote(riventarget());
                        ...
                        if (canq && !canhd && Utils.GameTimeTickCount - lasthd >= 300)
                        {
                            if (Utils.GameTimeTickCount - lastw >= 300 + Game.Ping)
                            {
                                ...
                                //player.Spellbook.CastSpell(SpellSlot.Q, riventarget().ServerPosition);
                            }
                        }
                    }
                }
            ...
            if (player.IsValid && orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LaneClear)
            {
                Clear();
                Wave();
            }

            if (player.IsValid && orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Flee)
            {
                Flee();
            }
        ...
        private static void OnMenuLoad()
        {
            ...
            var combom = new Menu("Combo", "combo");
            ...
            foreach (var hero in HeroManager.Enemies)
                newmenu.AddItem(new MenuItem("w" + hero.ChampionName, hero.ChampionName)).SetValue(false).SetTooltip("Only W if it will hit " + hero.ChampionName).DontSave();
            ...
            combom.AddSubMenu(r2menu);
            
            menu.AddSubMenu(combom);
...
//            TryIgnote(target);
                ...
                if (canq)
                {
                    if (menubool("safeq"))
                    {
                        if (endq.CountEnemiesInRange(200) <= 2)
                        {
                            ...
                            //player.Spellbook.CastSpell(SpellSlot.Q, target.ServerPosition);
                        }
                    }
        ...
        private static void Windslash()
        {
            ...
            {
                if (orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Burst && canburst())
                {
...
//                OrbTo(menubool("useaoeq") && GetCenterMinion().IsValidTarget() ? GetCenterMinion() : unit);
                ...
                if (menubool("semiq"))
                {
                    if (q.IsReady() && Utils.GameTimeTickCount - lastaa < 1200 && qtarg != null)
                    {
                            ...
                            orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Burst)
                        {
                            ...
                            //player.Spellbook.CastSpell(SpellSlot.Q, qtarg.ServerPosition);
                        }
            ...
            Obj_AI_Base.OnProcessSpellCast += (sender, args) =>
            {
                ...
                switch (args.SData.Name)
                {
                    case "ItemTiamatCleave":
                        ...
                        if (menulist("wsmode") == 1 || orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Burst)
                        {
                            if (orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo || orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Burst)
                            {
                                if (canburst() && uo)
                                {
                                    ...
                                    {
                                        if (!isteamfightkappa || menubool("r" + riventarget().ChampionName) ||
                                             isteamfightkappa && !rrektAny() || orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Burst)
                                        {
                    ...
                    case "RivenFeint":
                        ...
                        if (orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Flee)
                        {
                           ...
                           //     var btarg = EloBuddy.SDK.TargetSelector.GetTarget(r.Range, DamageType.Physical);
                        ...
                        if (orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
                        {
                            if (cc == 2 && !uo && r.IsReady() && riventarget() != null)
                            {
                                checkr();
                                Utility.DelayAction.Add(240 - Game.Ping, () => q.Cast(riventarget().ServerPosition));
                                //Utility.DelayAction.Add(240 - Game.Ping, () => //player.Spellbook.CastSpell(SpellSlot.Q, riventarget().ServerPosition));
                            }
            ...
            Interrupter2.OnInterruptableTarget += (sender, args) =>
            {
                ...
                if (menubool("qint") && q.IsReady() && cc >= 2)
                {
                    if (!sender.Position.UnderTurret(true))
                    {
                        ...
                        //player.Spellbook.CastSpell(SpellSlot.Q, sender.ServerPosition);
        ...
        private static void AuraUpdate()
        {
            if (!player.IsDead)
            {
                ...
                if (player.HasBuff("RivenTriCleave") && orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Burst)
                {
...
//                       Orbwalking.LastAATick = 0;
        ...
        private static void Drawings()
        {
            Drawing.OnDraw += args =>
            {
/*                checkBox5.OnValueChange += delegate (ValueBase<bool> sender, ValueBase<bool>.ValueChangeArgs args)
                {
                    Hacks.RenderWatermark = args.NewValue;
                };
                Hacks.RenderWatermark = checkBox5.CurrentValue;
//*/
                if (!player.IsDead)
                {
                    if (riventarget().IsValidTarget())
