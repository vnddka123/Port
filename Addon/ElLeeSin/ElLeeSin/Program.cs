namespace ElLeeSin
{
    ...
    internal static class Program
    {
        ...
        public static bool CastItems(Obj_AI_Base target)
        {
            ...
            var heroes = Player.GetEnemiesInRangeLS(385).Count;
            var count = heroes;
        ...
        private static void CastE()
        {
            ...
            if (EState)
            {
                var target = HeroManager.Enemies.Where(x => x.IsValidTargetLS(spells[Spells.E].Range + 20)).ToList();
            ...
            {
                var target =
                    HeroManager.Enemies.Where(i => HasEBuff(i) && i.IsValidTargetLS(spells[Spells.E].Range)).ToList();
                if (target.Count == 0)
                {
                    return;
                }
        ...
        {
            if (!spells[Spells.Q].IsReady() || !target.IsValidTargetLS(spells[Spells.Q].Range))
            {
                return;
            }
            ...
            if (prediction.Hitchance >= HitChance.High || prediction.Hitchance >= HitChance.Immobile)
            {
                if (prediction.CollisionObjects.Count == 0)
                {
                    spells[Spells.Q].Cast(prediction.CastPosition);
                     ...
                     && target.IsValidTargetLS(spells[Spells.Q].Range)
        ...
        private static void Combo()
        {
            ...
            if (ParamBool("ElLeeSin.Combo.Q2") && !Player.IsDashing() && target.HasQBuff()
                && target.IsValidTargetLS(1300f) && !isInQ2)
            {
                if ((castQAgain || spells[Spells.Q].GetDamage(target, 1) > target.Health + target.AttackShield)
                    ...
                    && !target.IsValidTargetLS(Orbwalking.GetRealAutoAttackRange(Player)))
                {
            ...
            if (spells[Spells.R].GetDamage(target) >= target.Health && ParamBool("ElLeeSin.Combo.KS.R")
                && target.IsValidTargetLS(spells[Spells.R].Range))
            {
                ...
                && (!ParamBool("ElLeeSin.Combo.W") || !ParamBool("ElLeeSin.Combo.Mode.WW")))
            {
                ...
                if (WState && target.IsValidTargetLS(Orbwalking.GetRealAutoAttackRange(Player)))
                {
                    ...
                    if (QState)
                    {
                        ...
                        var insectObjects =
                            HeroManager.Enemies.Where(
                                i =>
                                i.IsValidTargetLS(spells[Spells.Q].Range)
                                ...
                                .Where(
                                    m =>
                                    m.IsValidTargetLS(spells[Spells.Q].Range)
        ...
        private static void JungleClear()
        {
            try
            {
                ...
                if (spells[Spells.W].IsReady() && ParamBool("ElLeeSin.Jungle.W"))
                {
                    ...
                    if (WState && minion.IsValidTargetLS(Orbwalking.GetRealAutoAttackRange(Player)))
                    {
                        spells[Spells.W].Cast();
                        LastW = Environment.TickCount;
                    }
            ...
            try
            {
                return
                    ObjectManager.Get<Obj_AI_Base>()
                        .Where(a => a.IsValidTargetLS(1300))
