...
namespace OneKeyToWin_AIO_Sebby
{
    ...
    class Activator
    {
        ...
        {
            if (Player.LSInFountain() || Player.IsRecalling() || Player.IsDead)
            {
                return;
            }
            ...
            if (CanUse(smite) )
            {
                ...
                if (mobs.Count == 0 && (Player.LSGetSpellSlot("s5_summonersmiteplayerganker") != SpellSlot.Unknown || Player.LSGetSpellSlot("s5_summonersmiteduel") != SpellSlot.Unknown))
                {
                    ...
                    if (enemy.LSIsValidTarget())
                    {
                        ...
                        var smiteDmg = Player.GetSummonerSpellDamage(enemy, LeagueSharp.Common.Damage.SummonerSpell.Smite);
                ...
                if (mobs.Count > 0 && Config.Item("Smite").GetValue<KeyBind>().Active)
                {
                    foreach (var mob in mobs)
                    {
                        ...
                        if ((((mob.BaseSkinName.ToLower().Contains("dragon")) && Config.Item("Rdragon", true).GetValue<bool>())
                            ...
                            && mob.Health <= Player.GetSummonerSpellDamage(mob, LeagueSharp.Common.Damage.SummonerSpell.Smite))
                        {
        ...
        private void Ignite()
        {
            if (CanUse(ignite) && Config.Item("Ignite").GetValue<bool>())
            {
                foreach (var enemy in HeroManager.Enemies.Where(enemy => enemy.LSIsValidTarget(600)))
                {
                    ...
                    var IgnDmg = Player.GetSummonerSpellDamage(enemy, LeagueSharp.Common.Damage.SummonerSpell.Ignite);
