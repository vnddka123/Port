    ...
    class Vayne
    {
        ...
        private void BeforeAttack(LeagueSharp.Common.Orbwalking.BeforeAttackEventArgs args)
        {
            ...
            if (GetWStacks(t) < 2 && args.Target.Health > 5 * Player.LSGetAutoAttackDamage(t))
            {
                ...
                {
                    if (LeagueSharp.Common.Orbwalking.InAutoAttackRange(target) && args.Target.Health > 3 * Player.LSGetAutoAttackDamage(target))
                    {
                        args.Process = false;
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (Program.LagFree(1) && Q.LSIsReady())
            {
                ...
                if (Program.Combo && Config.Item("autoQ", true).GetValue<bool>() && !Config.Item("Qonly", true).GetValue<bool>())
                {
                    ...
                    if (t.LSIsValidTarget() && !LeagueSharp.Common.Orbwalking.InAutoAttackRange(t) && t.Position.LSDistance(Game.CursorPos) < t.Position.LSDistance(Player.Position) &&  !t.LSIsFacing(Player))
                    {
                        var dashPos = Dash.CastDash();
                        if (!dashPos.IsZero)
                        {
