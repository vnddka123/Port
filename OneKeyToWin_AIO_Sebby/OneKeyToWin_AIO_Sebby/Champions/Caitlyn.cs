...
namespace OneKeyToWin_AIO_Sebby
{
    class Caitlyn
    {
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (Program.LagFree(1) && E.LSIsReady() && LeagueSharp.Common.Orbwalking.CanMove(40))
                LogicE();
            if (Program.LagFree(2) && W.LSIsReady() && LeagueSharp.Common.Orbwalking.CanMove(40))
                LogicW();
            if (Program.LagFree(3) && Q.LSIsReady() && LeagueSharp.Common.Orbwalking.CanMove(40) && Config.Item("autoQ2", true).GetValue<bool>())
                LogicQ();
        ...
        private void LogicQ()
        {
            ...
            {
                if (GetRealDistance(t) > bonusRange() + 250 && !LeagueSharp.Common.Orbwalking.InAutoAttackRange(t) && OktwCommon.GetKsDamage(t, Q) > t.Health && Player.LSCountEnemiesInRange(400) == 0)
                {
