  ...
  //          EloBuddy.SDK.Orbwalker.OnPostAttack += Orbwalker_OnPostAttack;
        ...
        private void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            if (Player.Mana > RMANA + EMANA )
            {
                ...
                {
                    if (W.LSIsReady() && Config.Item("AGCW", true).GetValue<bool>())
                    {
        ...
        private void LogicQ()
        {
            ...
            if (t.LSIsValidTarget())
            {
                ...
                else
                {
                    ...
                    else if (qDmg + rDmg > t.Health && R.LSIsReady() && Player.Mana > RMANA + QMANA)
                    {
                        Program.CastSpell(Q, t);
        ...
        private void LogicW()
        {
            ...
            if (t.LSIsValidTarget())
            {
                ...
                {
                    if (!LeagueSharp.Common.Orbwalking.InAutoAttackRange(t) || Player.LSCountEnemiesInRange(300) > 0 || t.LSCountEnemiesInRange(250) > 1 || Player.HealthPercent < 50)
                        W.Cast(t, true, true);
        ...
        private void LogicE()
        {
            ...
            if (HeroManager.Enemies.Any(target => target.LSIsValidTarget(500) && target.IsMelee))
            {
        ...
        private void LogicR()
        {
            foreach (var target in HeroManager.Enemies.Where(target => target.LSIsValidTarget(R1.Range) && OktwCommon.ValidUlt(target)))
            {
