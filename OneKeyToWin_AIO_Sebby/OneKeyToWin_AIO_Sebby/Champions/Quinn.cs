    ...
    class Quinn
    {
        ...
        private void Orbwalking_BeforeAttack(LeagueSharp.Common.Orbwalking.BeforeAttackEventArgs args)
        {
            if(args.Target.Type == GameObjectType.AIHeroClient && Config.Item("focusP", true).GetValue<bool>() && args.Target.HealthPercent > 40)
            {
                ...
                {
                    var best = HeroManager.Enemies.FirstOrDefault(enemy => enemy.LSIsValidTarget() && LeagueSharp.Common.Orbwalking.InAutoAttackRange(enemy) && enemy.HasBuff("quinnw"));
                    if(best != null)
            ...
            {
                var bestMinion = Cache.GetMinions(Player.Position, Player.AttackRange).FirstOrDefault(minion => minion.LSIsValidTarget() && LeagueSharp.Common.Orbwalking.InAutoAttackRange(minion) && minion.HasBuff("quinnw"));
        ...
        {
            if (E.LSIsReady() && Config.Item("Int", true).GetValue<bool>() && sender.LSIsValidTarget(E.Range))
                E.CastOnUnit(sender);
        }
        ...
        private void afterAttack(AttackableUnit unit, AttackableUnit target)
        {
            if ( target.Type == GameObjectType.AIHeroClient)
            {
                ...
                if (E.LSIsReady() && Config.Item("autoE", true).GetValue<bool>() && t.LSIsValidTarget(E.Range) && t.LSCountEnemiesInRange(800) < 3)
                {
        ...
        {
            if (E.LSIsReady() && Config.Item("AGC", true).GetValue<bool>() && Config.Item("gap" + gapcloser.Sender.ChampionName,true).GetValue<bool>())
            {
        ...
        private void LogicR()
        {
            if (Player.LSInFountain() && R.Instance.Name == "QuinnR")
            {
                R.Cast();
            }
        }
